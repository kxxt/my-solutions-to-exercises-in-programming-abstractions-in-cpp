...
#else
/*static*/ std::string OS::getName() { return "unknown"; }

/*static*/ std::string OS::getVersion() { return "unknown"; }

bool OS::isLinux() { return false; }

bool OS::isMac() { return false; }
...
ErrorException::ErrorException(std::string msg) : _kind("error") {
  _msg = msg;
...
void ErrorException::dump() const { dump(std::cerr); }

void ErrorException::dump(std::ostream &out) const {
  out << std::endl;
  out << "***" << std::endl;
  out << "*** STANFORD C++ LIBRARY" << std::endl;
  out << "*** An ErrorException occurred during program execution:"
      << std::endl;
  if (!_msg.empty()) {
    out << ("*** " + _msg) << std::endl;
  }
...
bool ErrorException::hasStackTrace() const { return !_stackTrace.empty(); }
...
void ErrorException::setKind(const std::string &kind) { _kind = kind; }
 ...
 * @version 2019/04/16
 * - bug fix for stack trace line numbers on some Mac systems (thanks to Julie
 * Zelenski)
 * @version 2018/10/22
 * - bug fix for STL vector vs Stanford Vector
 * @version 2018/10/18
 * - added addr2line_functionName to resolve some function names not in
 * backtrace
 * - improved calculation of function offsets for better stack trace resolving
 * @version 2017/10/18
 * - small bug fix for pointer comparison
 * @version 2017/09/02
 * - small bug fix for new clang warning about ordered comparison between
 * pointer and zero
 * @version 2016/12/01
...
int execAndCapture(std::string cmd, std::string &output) {
  ...
  if (!CreateProcessA(nullptr,
    ...
    std::cerr << "CREATE PROCESS FAIL: " << platform::os_getLastError()
              << std::endl;
  ...
  if (!ReadFile(g_hChildStd_OUT_Rd, chBuf, BUFSIZE, &dwRead, nullptr) ||
      dwRead == 0) {
    return 1;
  }
  std::ostringstream out;
...
std::string addr2line_clean(std::string line) {
  ...
  // "ZN10stacktrace25print_stack_trace_windowsEv at
  // C:\Users\stepp\Documents\StanfordCPPLib\build\stanfordcpplib-windows-Desktop_Qt_5_3_MinGW_32bit-Debug/../../StanfordCPPLib/stacktrace/call_stack_windows.cpp:126"
#elif defined(__APPLE__)
  // Mac OS X version (atos)
  // "Vector<int>::checkIndex(int) const (in Autograder_QtCreatorProject)
  // (vector.h:764)"
...
std::string addr2line_functionName(std::string line) {
  ...
  // stanfordcpplib::autograder::GuiAutograder::runTest(stanfordcpplib::autograder::AutograderTest*)::{lambda()#1}>::_M_invoke(std::_Any_data
  // const&) at std_function.h:318"
  if (line.find(" at ") != std::string::npos) {
...
int addr2line_all(void **addrs, int length, std::string &output) {
  ...
  if (base) {
    out << "atos -l " << base << " -o "
        << exceptions::getProgramNameForStackTrace() << addrsStr;
  } else {
  ...
  if (fileExists("addr2line64.exe")) {
    out << "addr2line64.exe -f -i -C -s -p -e \""
        << exceptions::getProgramNameForStackTrace() << "\"" << addrsStr;
  } else {
...
call_stack::call_stack(const size_t /*num_discard = 0*/) {
  ...
  for (int i = STATIC_VARIABLE(STACK_FRAMES_TO_SKIP); i < stack_depth; i++) {
    ...
    // const char *dli_fname;   // pathname of shared object (file) that
    // contains address void       *dli_fbase;   // address at which shared
    // object is loaded in system memory const char *dli_sname;   // name of
    // nearest symbol with address lower than addr void       *dli_saddr;   //
    ...
    int status;
    char *demangled =
        abi::__cxa_demangle(symname, /* buffer */ nullptr,
    ...
    //        std::cout << "call_stack: I am thread " <<
    //        GThread::getCurrentThread()->objectName().toStdString() <<
    //        std::endl; std::cout << "info for " << trace[i] << ":" <<
    //        std::endl; std::cout << "dlinfo " << i << ":"
    //                  << " fbase=" << dlinfo.dli_fbase
    //                  << " fname=" << (dlinfo.dli_fname ? dlinfo.dli_fname :
    //                  "NULL")
    //                  << " sname=" << (dlinfo.dli_sname ? dlinfo.dli_sname :
    //                  "NULL")
    //                  << " saddr=" << dlinfo.dli_saddr
    //                  << std::endl;
    //        if (demangled) {
    //            std::cout << "demangled name " << i << ": " <<
    //            std::string(demangled) << " (status " << status << ")" <<
    //            std::endl;
    //        } else {
    //            std::cout << "demangled name " << i << ": NULL" << " (status "
    //            << status << ")" << std::endl;
    //        }
    //        std::cout << std::endl;
  ...
  for (int i = 0, size = (int)stack.size(); i < size; i++) {
    std::string opt1 =
        (2 * i < numAddrLines ? addr2lineLines[2 * i] : std::string());
    std::string opt2 =
        (2 * i + 1 < numAddrLines ? addr2lineLines[2 * i + 1] : std::string());
 ...
 * @version 2014/11/18
 * - fixed minor bug with filtering out nested <> template args from stack
 * traces
 * @version 2014/11/14
 * - fixed bug with SIGABRT handling in autograder mode (was muffling unit test
 * failures)
 * @version 2014/11/12
...
STATIC_CONST_VARIABLE_DECLARE_COLLECTION(Vector<int>, SIGNALS_HANDLED, SIGSEGV,
                                         SIGILL, SIGFPE, SIGINT)
#else
STATIC_CONST_VARIABLE_DECLARE_COLLECTION(Vector<int>, SIGNALS_HANDLED, SIGSEGV,
...
std::string cleanupFunctionNameForStackTrace(std::string function) {
  ...
  stringReplaceInPlace(function, "stanfordcpplib::collections::GenericSet",
                       "Set");
...
LONG WINAPI UnhandledException(LPEXCEPTION_POINTERS exceptionInfo) {
  ...
  // Please re-install Qt Creator with the proper compiler (MinGW 32-bit)
  // enabled.
#if _WIN64
...
void setTopLevelExceptionHandlerEnabled(bool enabled, bool force) {
  ...
  if ((!STATIC_VARIABLE(topLevelExceptionHandlerEnabled) || force) && enabled) {
    ...
    // also set up a signal handler for things like segfaults /
    // null-pointer-dereferences
    signalHandlerEnable();
  } else if ((STATIC_VARIABLE(topLevelExceptionHandlerEnabled) || force) &&
             !enabled) {
...
bool shouldFilterOutFromStackTrace(const std::string &function) {
  ...
  static const Vector<std::string> FORBIDDEN_SUBSTRINGS{
      " error(", "__cxa_rethrow", "__cxa_call_terminate",
      "__cxa_call_unexpected", "__func::", "__function::", "_endthreadex",
      "_Function_base::_Base_manager::", "_Function_handler", "_Internal_",
      "__invoke_impl", "__invoke_result::type", "__invoke_void", "__unexpected",
      "thread::_Invoker", "thread::_State_impl", "_M_invoke", "_sigtramp",
      "autograderMain", "BaseThreadInitThunk", "call_stack_gcc.cpp",
      "call_stack_windows.cpp", "CFRunLoopDoSource", "CFRunLoopRun",
      "CFRUNLOOP_IS", "crtexe.c", "decltype(forward",
      "ErrorException::ErrorException", "exceptions.cpp", "function::operator",
      "GetModuleFileName", "GetProfileString",
      // "GStudentThread::run",
      "GThreadQt::run", "GThreadQt::start", "GThreadStd::run",
      "GThreadStd::start", "InitializeExceptionChain", "KnownExceptionFilter",
      "M_invoke", "multimain.cpp",
      // "operator",
      "pthread_body", "pthread_start", "printStackTrace",
      // "QAbstractItemModel::",
      // "QAbstractProxyModel::",
      "QApplication::notify", "QApplicationPrivate::", "QCoreApplication::",
      "QGuiApplicationPrivate::", "QMetaMethod::", "QMetaObject::",
      "QObjectPrivate::", "qt_plugin_instance",
      "QtGui::startBackgroundEventLoop",
      // "QWidget::",
      "QWidgetBackingStore::", "QWindowSystemInterface::",
      "require::_errorMessage", "RunCurrentEventLoopInMode",
      "shouldFilterOutFromStackTrace",
      "stacktrace::", "stanfordCppLibPosixSignalHandler",
      "stanfordCppLibSignalHandler", "stanfordCppLibTerminateHandler",
      "stanfordCppLibUnexpectedHandler", "testing::", "UnhandledException",
      "WinMain@"};
...
void printStackTrace(std::ostream &out) {
  // constructing the following object jumps into fancy code in
  // call_stack_gcc/windows.cpp to rebuild the stack trace; implementation
  // differs for each operating system
  ...
  for (int i = 0; i < entries.size(); ++i) {
    ...
    if (!STATIC_VARIABLE(STACK_TRACE_SHOULD_FILTER) ||
      ...
      funcNameLength =
          std::max(funcNameLength, (int)entries[i].function.length());
      entriesToShowCount++;
    }
  }
  ...
  if (lineStrLength > 0) {
    ...
    if (STATIC_VARIABLE(STACK_TRACE_SHOW_TOP_BOTTOM_BARS)) {
      ...
      lineout << "*** " << std::setw(lineStrLength) << std::left << "file:line"
              << "  "
              << "function" << std::endl
              << "*** " << std::string(lineStrLength + 2 + funcNameLength, '=')
              << std::endl;
  ...
  for (int i = 0; i < entries.size(); ++i) {
    ...
    // show Main() as main() to hide hidden case-change by Stanford C++ lib
    // internals
    ...
    if (entry.function == "main" || entry.function == "main()" ||
        entry.function == "main(int, char**)" || entry.function == "qMain" ||
        entry.function == "qMain()") {
      break;
    }
  }
  if (entries.size() == 1 && fakeStackPtr &&
      entries[0].address == fakeStackPtr) {
...
#define FILL_IN_EXCEPTION_TRACE(ex, kind, desc)                                \
  {                                                                            \
    ...
    if ((!__kind.empty())) {                                                   \
      stringReplaceInPlace(msg, DEFAULT_EXCEPTION_KIND, __kind);               \
    }                                                                          \
    if ((!__desc.empty())) {                                                   \
      stringReplaceInPlace(msg, DEFAULT_EXCEPTION_DETAILS, __desc);            \
    }                                                                          \
    out << msg;                                                                \
    out.flush();                                                               \
    printStackTrace(out);                                                      \
...
#ifndef _WIN32
void stanfordCppLibPosixSignalHandler(int sig, siginfo_t * /*siginfo*/,
                                      void * /*context*/) {
...
static void stanfordCppLibSignalHandler(int sig) {
  ...
  if (sig == SIGSEGV) {
    ...
    SIGNAL_DETAILS = "This typically happens when you try to dereference a "
    ...
    SIGNAL_DETAILS =
        "This typically happens when you divide by 0 or produce an overflow.";
  } else if (sig == SIGINT) {
    SIGNAL_KIND = "An interrupt error (SIGINT)";
    SIGNAL_DETAILS = "This typically happens when your code timed out because "
                     "it was stuck in an infinite loop.";
  } else if (sig == SIGSTACK) {
    SIGNAL_KIND = "A stack overflow";
    SIGNAL_DETAILS = "This can happen when you have a function that calls "
                     "itself infinitely.";
  } else if (sig == SIGUSR1) {
    SIGNAL_KIND = "Custom signal 1";
    SIGNAL_DETAILS =
        "This can happen when you produce infinite output in your code.";
  }
  ...
  std::cerr << (std::string("*** ") + SIGNAL_KIND +
  ...
  //        std::cerr << "*** (unable to print stack trace because of stack
  //        memory corruption.)" << std::endl; std::cerr << "*** " << line <<
  //        std::endl;
  //    }
  std::cerr.flush();
...
[[noreturn]] static void stanfordCppLibTerminateHandler() {
  ...
  try {
    ...
    if (ex.hasStackTrace()) {
      ...
      FILL_IN_EXCEPTION_TRACE(ex, "An ErrorException",
                              insertStarsBeforeEachLine(ex.what()));
    }
  } catch (const std::exception &ex) {
    FILL_IN_EXCEPTION_TRACE(ex, "A C++ exception",
                            insertStarsBeforeEachLine(ex.what()));
  } catch (const std::string &str) {
    FILL_IN_EXCEPTION_TRACE(str, "A string exception",
                            insertStarsBeforeEachLine(str));
...
void injectAddr2lineInfo(entry &ent, const std::string &line) {
  ...
  if (startsWith(ent.function, "_Z")) {
    int status;
    char *demangled =
        abi::__cxa_demangle(ent.function.c_str(), nullptr, nullptr, &status);
    if (status == 0 && demangled) {
...
call_stack::call_stack(const size_t /*num_discard = 0*/) {
  ...
  if (fakeStackPtr) {
    ...
    if (exceptionInfo->ExceptionRecord->ExceptionCode ==
      ...
      while ((int)traceVector.size() < STATIC_VARIABLE(STACK_FRAMES_MAX) &&
             StackWalk(IMAGE_FILE_MACHINE_I386, process, thread, &frame,
    ...
    if (!::SymSetOptions(
            ...
            // | SYMOPT_UNDNAME
            | SYMOPT_CASE_INSENSITIVE | SYMOPT_LOAD_LINES)) {
    ...
    if (!::SymLoadModule(
            ...
            BaseAddr, // Base address of the module (cannot be null if .PDB file
                      // is used, otherwise it can be null)
            FileSize)) { // Size of the file (cannot be null if .PDB file is
                         // used, otherwise it can be null)
                         // std::cerr << "Error: SymLoadModule() failed: " <<
      // platform::os_getLastError() << std::endl; return;
    }
  }

  // let's also try to get the line numbers via an external command-line process
  // 'addr2line' (ought to be able to get this information through C function
  // 'backtrace', but for some reason, Qt Creator's shipped version of MinGW
  // does not include this functionality, argh)
  std::string addr2lineOutput;
  ...
  for (int i = 0; i < (int)traceVector.size(); ++i) {
    ...
    if (process &&
        ::SymFromAddr(process, (DWORD64)traceVector[i], nullptr, symbol)) {
...
long Timer::elapsed() const { return _stopMS - _startMS; }

bool Timer::isStarted() const { return _isStarted; }
...
Note::Note(double duration, Pitch pitch, int octave, Accidental accidental,
           bool repeat) {
...
bool Note::equals(const Note &note2) const {
  return floatingPointEqual(duration, note2.duration) && pitch == note2.pitch &&
         octave == note2.octave && accidental == note2.accidental &&
         repeat == note2.repeat;
}

Note::Accidental Note::getAccidental() const { return accidental; }

double Note::getDuration() const { return duration; }

int Note::getOctave() const { return octave; }

Note::Pitch Note::getPitch() const { return pitch; }

bool Note::isRepeat() const { return repeat; }

bool Note::isRest() const { return pitch == R; }
...
void Note::setRepeat(bool repeat) { this->repeat = repeat; }
...
int hashCode(const Note &note) {
  return hashCode(note.getDuration(), note.getPitch(), note.getOctave(),
...
/*static*/ void Sound::playSound(const std::string &filename) {
  ...
  GThread::runOnQtGuiThreadAsync([absPath]() {
    _qmediaPlayer->setMedia(
        QUrl::fromLocalFile(QString::fromStdString(absPath)));
    _qmediaPlayer->play();
  });
}
...
Sound::Sound(std::string filename) : _filename(filename) { initialize(); }

Sound::~Sound() {
  // TODO
}

void Sound::play() { playSound(_filename); }
...
void computeMemoryDistances(void *const p, uintptr_t &stackDist,
                            uintptr_t &heapDist, uintptr_t &staticDist) {
  static int si = 0;
  int i = 0;
  int *pi = new int(0);

  // don't use abs/subtraction here because it can over/underflow with very
  // large integers
  stackDist =
      p > &i ? reinterpret_cast<uintptr_t>(p) - reinterpret_cast<uintptr_t>(&i)
             : reinterpret_cast<uintptr_t>(&i) - reinterpret_cast<uintptr_t>(p);
  heapDist =
      p > pi ? reinterpret_cast<uintptr_t>(p) - reinterpret_cast<uintptr_t>(pi)
...
void randomFeedBool(bool value) { STATIC_VARIABLE(fixedBools).push(value); }
...
void randomFeedInteger(int value) { STATIC_VARIABLE(fixedInts).push(value); }

void randomFeedReal(double value) { STATIC_VARIABLE(fixedReals).push(value); }
} // namespace autograder
/* end 'fixed' internal stuff */

bool randomBool() { return randomChance(0.5); }
...
int getRecursionIndentLevel() {
  ...
  for (int i = 0; i < entries.size(); ++i) {
    ...
    if (exceptions::shouldFilterOutFromStackTrace(entries[i].function) ||
        ...
        entries[i].function.find("getRecursionIndentLevel(") !=
            std::string::npos) {
      continue;
...
Direction leftFrom(Direction dir) { return Direction((dir + 3) % 4); }

Direction rightFrom(Direction dir) { return Direction((dir + 1) % 4); }

Direction opposite(Direction dir) { return Direction((dir + 2) % 4); }
...
std::string implode(const Vector<std::string> &v,
...
std::string trimToHeight(const std::string &s, int height,
  ...
  if (wasTooTall && !suffix.empty()) {
    lines.add(suffix + " (" + std::to_string(lineCount - height) +
              " line(s) truncated)");
  }
  return implode(lines);
}

std::string trimToSize(const std::string &s, int width, int height,
                       const std::string &suffix) {
...
std::string trimToWidth(const std::string &s, int width,
                        const std::string &suffix) {
  Vector<std::string> lines = explodeLines(s);
...
std::string diff(std::string s1, std::string s2, int flags) {
  ...
  while (index1 < lines1.size() && index2 < lines2.size()) {
    ...
    while ((s1 + s2 - index1 - index2) < (best1 + best2 - index1 - index2)) {
      ...
      if (d >= s1 &&
          (d + s2 - index1 - index2) < (best1 + best2 - index1 - index2)) {
        best1 = d;
        best2 = s2;
      }
      ...
      if (d >= s2 &&
          (d + s1 - index1 - index2) < (best1 + best2 - index1 - index2)) {
        best1 = s1;
        best2 = d;
      }
  ...
  for (int action : actions) {
    ...
    if (op > 0) {
      ...
      std::string xstr =
          std::string("") +
          (multipleLines ? (std::to_string(x0 + 1) + "-" + std::to_string(x1))
                         : std::to_string(x1));
      std::string ystr =
          std::string("") +
          ((y1 != y0 + 1) ? (std::to_string(y0 + 1) + "-" + std::to_string(y1))
                          : std::to_string(y1));
      std::string linesStr =
          std::string("\nLine") + (multipleLines ? "s " : " ");
      std::string doStr = std::string("do") + (multipleLines ? "" : "es");
      if (op == 1) {
        out += linesStr + xstr + " deleted near student line " +
               std::to_string(y1);
      } else if (op == 3) {
        if (xstr == ystr) {
      ...
      if (op == 2) {
        if (!(flags & IGNORE_LEADING) || x1 > 0) {
          out +=
              linesStr + std::to_string(x1) + " added at student line " + ystr;
        }
      } else if (op == 3) {
        // out += "---";
      }
...
void regexMatchCountWithLines(const std::string & /*s*/,
...
std::string regexReplace(const std::string &s, const std::string & /*regexp*/,
                         const std::string & /*replacement*/, int /*limit*/) {
...
// this function can be implemented the same way whether regexes are available
// or not
...
//    }
//    _ePowerLen = i;
//    BigFloat* ePowerBuf = new BigFloat[static_cast<unsigned
//    int>(_ePowerLen-_ePowerNeg)]; _ePower = &ePowerBuf[-_ePowerNeg];
//    _ePower[0].Copy(_e);
//    BigFloat* eInvPowerBuf = new BigFloat[static_cast<unsigned
//    int>(_ePowerLen-_ePowerNeg)]; _eInvPower = &eInvPowerBuf[-_ePowerNeg];
...
//         else if (IsInf())
//             return _isNegative ? -std::numeric_limits<double>::infinity() :
...
//// print an accurate initializer for a double: (n / d) * 2^^x, where 1 <= n/d
///< 2
...
//    // recurse until we get too precise: 1/r2 = c + r3, and n3/d3 = (c*n2 +
//    n1)/(c*d2 + d1) for (;;)
//    {
//        r.Invert();
//        BigFloat c(r);
...
//        {
//            printf("%sstatic_cast<double>(%llu) / %llu", sign, n2.ToInteger(),
//            d2.ToInteger()); break;
//        }
...
//        {
//            printf("%sstatic_cast<double>(%llu) / %llu", sign, n1.ToInteger(),
//            d1.ToInteger()); break;
//        }
//    }
...
//                while (iPos >= stopB)
//                {
//					// the low-order digit is nonzero, and
// carry==false, so it is going to be in range.
//					// all the other digits might be zero, but
//they have carry==true, so they will also be in range.
//                    _d[iPos] = static_cast<u4>(c_range - b->_d[iPos-delta] -
//                    carry); carry = true; iPos--;
//                }
//            }
...
//	// need at least one full previous digit, and one full digit of zeros in
// front
...
//                {
//                    s8 p = t[i+j] - q*d[j];
//                    ASSERT(p < (((s8)1)<<(3*c_log/2)) && p >
//                    -(((s8)1)<<(3*c_log/2))); t[i+j-1] += p >> (c_log/2);
//                    t[i+j] = p & ((1<<(c_log/2))-1);
//                }
//            }
//        }
...
//    BigFloat limit(3217);
//    limit.Div(4096);    // limit is a little bigger than pi/4, there are
//    sometimes rounding issues ASSERT(CompareAbsolute(limit) <= 0); return
//    quadrant;
//}
...
//    // multiply the result by 1<<c_iter to make up for all those half-angle
//    reductions earlier Mult(1<<c_iter); if (flip)
//    {
//        Negate();
//        if (_isNegative)
//        {
...
//        while (i < BigFloatCache::_ePowerLen &&
//        Compare(BigFloatCache::_ePower[i]) >= 0)
//            ++i;
...
//                whole._d[whole._exponent - (i-offset)/c_log - minDigit] += (1
//                << ((i+offset)%c_log));
//            }
//        }
//    }
...
//        s8 i=BigFloatCache::_ePowerNeg+1;
//        while (i < BigFloatCache::_ePowerLen &&
//        Compare(BigFloatCache::_eInvPower[i]) <= 0)
//            ++i;
//        whole._exponent = minDigit + (i-1-offset)/c_log;
 ...
 * File: biginteger.cpp
 * --------------------
 * This file implements members of a class for arbitrary-size integer
 * arithmetic. It is meant to help get around the max/min value limit for types
...
BigInteger::BigInteger(const std::string &s, int radix) { setValue(s, radix); }
...
BigInteger::BigInteger(long n) { setValue(toString(n)); }

BigInteger BigInteger::abs() const { return BigInteger(getNumber()); }
...
std::pair<std::string, long> BigInteger::divide(const std::string &n,
                                                long den) {
  if (den == 0) {
...
const std::string &BigInteger::getNumber() const { return number; }

bool BigInteger::getSign() const { return sign; }

bool BigInteger::greater(const BigInteger &n1, const BigInteger &n2) {
  return less(n2, n1);
}

bool BigInteger::isInt() const { return stringIsInteger(toString()); }

bool BigInteger::isLong() const { return stringIsLong(toString()); }
...
bool BigInteger::isPositive() const { return !sign && number[0] != '0'; }
...
std::string BigInteger::stripNumberPrefix(const std::string &num, int radix) {
  std::string result;
  if (radix == 2 && (int)num.length() >= 2 && num[0] == '0' &&
      tolower(num[1]) == 'b') {
    result = num.substr(2);
  } else if (radix == 8 && (int)num.length() >= 2 && num[0] == '0' &&
             tolower(num[1]) == 'o') {
    result = num.substr(2);
  } else if (radix == 16 && (int)num.length() >= 2 && num[0] == '0' &&
             tolower(num[1]) == 'x') {
    result = num.substr(2);
  } else {
    result = num;
...
int BigInteger::toInt() const { return (int)toLong(number); }

long BigInteger::toLong() const { return toLong(number); }
...
BigInteger::operator bool() const { return *this != ZERO; }
...
BigInteger::operator int() const { return toInt(); }

BigInteger::operator long() const { return toLong(); }
...
BigInteger operator|(const BigInteger &b1, const BigInteger &b2) {
  ...
  // don't need to check for leading 0s because | can only make more 1s than
  // before s1 = removeLeadingZeros(s1);
...
bool operator>=(const BigInteger &b1, const BigInteger &b2) {
  return BigInteger::equals(b1, b2) || BigInteger::greater(b1, b2);
}

bool operator<=(const BigInteger &b1, const BigInteger &b2) {
  return BigInteger::equals(b1, b2) || BigInteger::less(b1, b2);
}
...
static void _errorMessage(const std::string &caller,
...
#define _default(value, defaultValue)                                          \
  ((value) == std::string("") ? (defaultValue) : (value))

void inRange(double value, double min, double max, const std::string &caller,
             const std::string &valueName, const std::string &details) {
  _spl_assert(
      min <= value && value <= max, caller, _default(valueName, "value"),
      _default(details, " must be between " + std::to_string(min) + " and " +
                            std::to_string(max) + " inclusive but was " +
                            std::to_string(value)));
}

void inRange(int value, int min, int max, const std::string &caller,
             const std::string &valueName, const std::string &details) {
  _spl_assert(
      min <= value && value <= max, caller, _default(valueName, "value"),
      _default(details, " must be between " + std::to_string(min) + " and " +
...
void inRange2D(double x, double y, double maxX, double maxY,
               const std::string &caller, const std::string &xValueName,
               const std::string &yValueName, const std::string &details) {
  inRange2D(x, y, /* minX */ 0.0, /* minY */ 0.0, maxX, maxY, caller,
            xValueName, yValueName, details);
}

void inRange2D(double x, double y, double minX, double minY, double maxX,
               double maxY, const std::string &caller,
               const std::string &xValueName, const std::string &yValueName,
               const std::string &details) {
  inRange(x, minX, maxX, caller, xValueName,
          _default(details,
                   " must be between (" + std::to_string(minX) + "," +
                       std::to_string(minY) + ")-" + std::to_string(maxX) +
                       "," + std::to_string(maxY) + ") inclusive but was (" +
                       std::to_string(x) + "," + std::to_string(y) + ")"));
  inRange(y, minY, maxY, caller, yValueName,
          _default(details,
                   " must be between (" + std::to_string(minX) + "," +
                       std::to_string(minY) + ")-" + std::to_string(maxX) +
                       "," + std::to_string(maxY) + ") inclusive but was (" +
...
void inRange2D(int x, int y, int maxX, int maxY, const std::string &caller,
               const std::string &xValueName, const std::string &yValueName,
               const std::string &details) {
  inRange2D(x, y, /* minX */ 0, /* minY */ 0, maxX, maxY, caller, xValueName,
            yValueName, details);
}

void inRange2D(int x, int y, int minX, int minY, int maxX, int maxY,
               const std::string &caller, const std::string &xValueName,
               const std::string &yValueName, const std::string &details) {
  inRange(x, minX, maxX, caller, xValueName,
          _default(details,
...
void nonEmpty(const std::string &str, const std::string &caller,
              const std::string &valueName, const std::string &details) {
  _spl_assert(!str.empty(), caller, _default(valueName, "string"),
              _default(details, " must not be empty"));
}

void nonNegative(double value, const std::string &caller,
                 const std::string &valueName, const std::string &details) {
  _spl_assert(value >= 0.0, caller, _default(valueName, "value"),
              _default(details, " must be non-negative but was " +
                                    std::to_string(value)));
}

void nonNegative(int value, const std::string &caller,
                 const std::string &valueName, const std::string &details) {
  _spl_assert(value >= 0, caller, _default(valueName, "value"),
              _default(details, " must be non-negative but was " +
                                    std::to_string(value)));
}

void nonNegative(long value, const std::string &caller,
                 const std::string &valueName, const std::string &details) {
  _spl_assert(value >= 0, caller, _default(valueName, "value"),
              _default(details, " must be non-negative but was " +
                                    std::to_string(value)));
...
void nonNegative2D(double x, double y, const std::string &caller,
                   const std::string &xValueName, const std::string &yValueName,
                   const std::string &details) {
...
void nonNegative2D(int x, int y, const std::string &caller,
                   const std::string &xValueName, const std::string &yValueName,
                   const std::string &details) {
  nonNegative(x, caller, xValueName, details);
  nonNegative(y, caller, yValueName, details);
...
void nonNull(const void *ptr, const std::string &caller,
             const std::string &valueName, const std::string &details) {
  _spl_assert(ptr != nullptr, caller, _default(valueName, "value"),
              _default(details, " must be non-null"));
}

void positive(double value, const std::string &caller,
              const std::string &valueName, const std::string &details) {
  _spl_assert(
      value > 0.0, caller, _default(valueName, "value"),
      _default(details, " must be positive but was " + std::to_string(value)));
}

void positive(int value, const std::string &caller,
              const std::string &valueName, const std::string &details) {
  _spl_assert(
      value > 0, caller, _default(valueName, "value"),
      _default(details, " must be positive but was " + std::to_string(value)));
}
...
double vectorDistance(double x, double y) { return sqrt(x * x + y * y); }
...
double Complex::abs() const { return sqrt(a * a + b * b); }

double Complex::imag() const { return b; }

double Complex::real() const { return a; }
...
int hashCode(const Complex &c) { return hashCode(c.real(), c.imag()); }
...
bool operator>(const Complex &c1, const Complex &c2) { return c2 < c1; }
 ...
 * @version 2016/11/07
 * - bug fix: urlDecode throws error on invalid encodings (courtesy GitHub
 * @scinart)
 * @version 2016/10/30
 * - alphabetized functions
 * - added overloads that take type char instead of string:
 *   stringContains, stringIndexOf, stringJoin, stringLastIndexOf,
 * stringReplace, stringSplit, toLowerCase, toUpperCase
 * @version 2016/10/13
...
std::string boolToString(int b) { return (b ? "true" : "false"); }
...
std::string doubleToString(double d) { return realToString(d); }
...
bool stringIsDouble(const std::string &str) { return stringIsReal(str); }
...
std::string stringJoin(const Vector<std::string> &v,
                       const std::string &delimiter) {
  if (v.isEmpty()) {
    return "";
  } else {
...
std::string stringReplace(const std::string &str, char old, char replacement,
                          int limit) {
  std::string str2 = str;
...
std::string stringReplace(const std::string &str, const std::string &old,
                          const std::string &replacement, int limit) {
  std::string str2 = str;
  stringReplaceInPlace(str2, old, replacement, limit);
  return str2;
...
int stringReplaceInPlace(std::string &str, char old, char replacement,
                         int limit) {
  int count = 0;
...
int stringReplaceInPlace(std::string &str, const std::string &old,
                         const std::string &replacement, int limit) {
  int count = 0;
  size_t startIndex = 0;
...
Vector<std::string> stringSplit(const std::string &str,
                                const std::string &delimiter, int limit) {
  std::string str2 = str;
  Vector<std::string> result;
  int count = 0;
  size_t index = 0;
...
char stringToChar(const std::string &str) {
  ...
  if ((int)str2.length() != 1) {
    error(
        "stringToChar: string must contain exactly 1 non-whitespace character");
  }
  return str2[0];
}

double stringToDouble(const std::string &str) { return stringToReal(str); }
...
std::string urlDecode(const std::string &str) {
  ...
  for (std::string::const_iterator i = str.begin(), n = str.end(); i != n;
    ...
    if (isalnum(c) || c == '-' || c == '_' || c == '.' || c == '~' ||
      ...
      msg << "urlDecode: Unexpected character in string: " << (int)c << " ("
          << c << ")";
      error(msg.str());
    }
  }
...
namespace std {
bool stob(const std::string &str) { return ::stringToBool(str); }

char stoc(const std::string &str) { return ::stringToChar(str); }

std::string to_string(bool b) { return ::boolToString(b); }

std::string to_string(char c) { return ::charToString(c); }

std::string to_string(void *p) { return ::pointerToString(p); }
} // namespace std
...
int hashCode(const Point &pt) { return hashCode(pt.getX(), pt.getY()); }
...
IntRange::IntRange(int length) : _min(0), _max(length - 1) {
  // empty
}
...
bool IntRange::contains(int n) const { return _min <= n && n <= _max; }

bool IntRange::contains(const IntRange &r) const {
  return _min <= r._min && _max >= r._max;
}

bool IntRange::empty() const { return _max < _min; }
...
bool IntRange::isEmpty() const { return _max < _min; }
...
int IntRange::max() const { return _max; }

int IntRange::min() const { return _min; }

int IntRange::size() const { return length(); }
...
IntRange range(int length) { return IntRange(length); }

IntRange range(int min, int max) { return IntRange(min, max); }

bool operator<(const IntRange &r1, const IntRange &r2) {
  return r1.min() < r2.min() || (r1.min() == r2.min() && r1.max() < r2.max());
}
...
bool operator!=(const IntRange &r1, const IntRange &r2) { return !(r1 == r2); }

bool operator>(const IntRange &r1, const IntRange &r2) { return r2 < r1; }
...
IntRange2D::IntRange2D(int width, int height, bool yMajor)
    : _minX(0), _minY(0), _maxX(width - 1), _maxY(height - 1), _yMajor(yMajor) {
  // empty
}

IntRange2D::IntRange2D(int minX, int minY, int maxX, int maxY, bool yMajor)
    : _minX(minX), _minY(minY), _maxX(maxX), _maxY(maxY), _yMajor(yMajor) {
  // empty
}
...
bool IntRange2D::empty() const { return _maxX < _minX || _maxY < _minY; }
...
bool IntRange2D::isEmpty() const { return _maxX < _minX || _maxY < _minY; }

bool IntRange2D::isYMajor() const { return _yMajor; }

int IntRange2D::maxX() const { return _maxX; }

int IntRange2D::maxY() const { return _maxY; }

int IntRange2D::minX() const { return _minX; }

int IntRange2D::minY() const { return _minY; }

int IntRange2D::size() const { return width() * height(); }
...
bool operator==(const IntRange2D &r1, const IntRange2D &r2) {
  return r1.minX() == r2.minX() && r1.minY() == r2.minY() &&
         r1.maxX() == r2.maxX() && r1.maxY() == r2.maxY() &&
...
bool operator>(const IntRange2D &r1, const IntRange2D &r2) { return r2 < r1; }
...
std::ostream &operator<<(std::ostream &out, const IntRange2D &r) {
  return out << "[" << r.minX() << "," << r.minY() << " .. " << r.maxX() << ","
             << r.maxY() << "]";
}
...
GChooser::GChooser(const std::initializer_list<std::string> &items,
                   QWidget *parent) {
  GThread::runOnQtGuiThread([this, parent]() {
    _iqcomboBox = new _Internal_QComboBox(this, getInternalParent(parent));
  });
...
void GChooser::addItem(const std::string &item) {
  ...
  GThread::runOnQtGuiThread(
      [this, item]() { _iqcomboBox->addItem(QString::fromStdString(item)); });
}
...
void GChooser::checkIndex(const std::string &member, int index, int min,
                          int max) const {
  if (max < 0) {
    max = size() - 1;
  }
...
void GChooser::clearItems() {
  GThread::runOnQtGuiThread([this]() { _iqcomboBox->clear(); });
}
...
std::string GChooser::getActionEventType() const { return "change"; }

_Internal_QWidget *GChooser::getInternalWidget() const { return _iqcomboBox; }
...
int GChooser::getItemCount() const { return _iqcomboBox->count(); }

int GChooser::getSelectedIndex() const { return _iqcomboBox->currentIndex(); }

std::string GChooser::getSelectedItem() const {
  return _iqcomboBox->currentText().toStdString();
}

std::string GChooser::getType() const { return "GChooser"; }

QWidget *GChooser::getWidget() const {
  return static_cast<QWidget *>(_iqcomboBox);
}

bool GChooser::isEditable() const { return _iqcomboBox->isEditable(); }

bool GChooser::isEmpty() const { return getItemCount() == 0; }
...
void GChooser::setSelectedIndex(int index) {
  ...
  GThread::runOnQtGuiThread(
      [this, index]() { _iqcomboBox->setCurrentIndex(index); });
}

void GChooser::setEditable(bool editable) {
  GThread::runOnQtGuiThread(
      [this, editable]() { _iqcomboBox->setEditable(editable); });
}
...
int GChooser::size() const { return _iqcomboBox->count(); }

_Internal_QComboBox::_Internal_QComboBox(GChooser *gchooser, QWidget *parent)
    : QComboBox(parent), _gchooser(gchooser) {
  require::nonNull(gchooser, "_Internal_QComboBox::constructor");
  setObjectName(QString::fromStdString("_Internal_QComboBox_" +
                                       std::to_string(gchooser->getID())));
  connect(this, SIGNAL(currentIndexChanged(int)), this, SLOT(handleChange()));
}

void _Internal_QComboBox::detach() { _gchooser = nullptr; }
...
GTextField::GTextField(const std::string &text, int charsWide, QWidget *parent)
    : _iqlineedit(nullptr), _iqspinbox(nullptr), _iqdoublespinbox(nullptr) {
  GThread::runOnQtGuiThread([this, parent]() {
...
GTextField::GTextField(int value, int min, int max, int step, QWidget *parent) {
  require::require(min <= max, "GTextField::constructor",
  ...
  GThread::runOnQtGuiThread([this, value, min, max, step, parent]() {
    _iqspinbox =
        new _Internal_QSpinBox(this, min, max, step, getInternalParent(parent));
...
GTextField::GTextField(double value, double min, double max, double step,
                       QWidget *parent) {
  require::require(min <= max, "GTextField::constructor",
                   "min (" + std::to_string(min) +
                       ") cannot be greater than max (" + std::to_string(max) +
  ...
  require::inRange(value, min, max, "GTextField::constructor", "value");
  GThread::runOnQtGuiThread([this, value, min, max, step, parent]() {
    _iqdoublespinbox = new _Internal_QDoubleSpinBox(this, min, max, step,
...
std::string GTextField::getActionEventType() const { return "change"; }
...
GTextField::InputType GTextField::getInputType() const { return _inputType; }
...
int GTextField::getMaxLength() const {
  ...
  if (_inputType == GTextField::INPUT_TYPE_TEXT) {
    ...
    maxLength = std::max(
        maxLength,
        (int)maxStr.length()); // TODO: may be incorrect w/ decimal value
  }
...
std::string GTextField::getType() const { return "GTextField"; }

std::string GTextField::getValue() const { return getText(); }
...
int GTextField::getValueAsInt() const { return getValueAsInteger(); }
...
void GTextField::setAutocompleteList(
...
void GTextField::setAutocompleteList(const Vector<std::string> &strings) {
  GThread::runOnQtGuiThread([this, &strings]() {
    ...
    QStringListModel *model =
        new QStringListModel(stringList, _iqlineedit); // TODO: MEMORY LEAK
    QCompleter *completer =
        new QCompleter(model, _iqlineedit); // TODO: MEMORY LEAK
...
void GTextField::setPlaceholder(const std::string &text) {
  GThread::runOnQtGuiThread([this, text]() {
    if (_inputType == GTextField::INPUT_TYPE_TEXT) {
      ...
      _iqdoublespinbox->lineEdit()->setPlaceholderText(
...
void GTextField::setValue(bool value) { setText(boolToString(value)); }

void GTextField::setValue(char value) { setText(charToString(value)); }

void GTextField::setValue(double value) { setText(realToString(value)); }

void GTextField::setValue(int value) { setText(std::to_string(value)); }

void GTextField::setValue(const std::string &value) { setText(value); }

bool GTextField::valueIsBool() const { return stringIsBool(trim(getText())); }

bool GTextField::valueIsChar() const { return (int)getText().length() == 1; }

bool GTextField::valueIsDouble() const {
  return stringIsDouble(trim(getText()));
}

bool GTextField::valueIsInt() const { return stringIsInteger(trim(getText())); }

bool GTextField::valueIsInteger() const {
  return stringIsInteger(trim(getText()));
}

bool GTextField::valueIsReal() const { return stringIsReal(trim(getText())); }

_Internal_QLineEdit::_Internal_QLineEdit(GTextField *gtextField,
                                         QWidget *parent)
    : QLineEdit(parent), _gtextfield(gtextField) {
  require::nonNull(gtextField, "_Internal_QLineEdit::constructor");
  setObjectName(QString::fromStdString("_Internal_QLineEdit_" +
                                       std::to_string(gtextField->getID())));
  connect(this, SIGNAL(textChanged(QString)), this,
          SLOT(handleTextChange(const QString &)));
}

void _Internal_QLineEdit::detach() { _gtextfield = nullptr; }
...
_Internal_QSpinBox::_Internal_QSpinBox(GTextField *gtextField, int min, int max,
                                       int step, QWidget *parent)
    : QSpinBox(parent), _gtextfield(gtextField) {
  require::nonNull(gtextField, "_Internal_QSpinBox::constructor");
  setObjectName(QString::fromStdString("_Internal_QSpinBox_" +
                                       std::to_string(gtextField->getID())));
  setRange(min, max);
  setSingleStep(step);
}

void _Internal_QSpinBox::detach() { _gtextfield = nullptr; }
...
QLineEdit *_Internal_QSpinBox::lineEdit() const { return QSpinBox::lineEdit(); }
...
_Internal_QDoubleSpinBox::_Internal_QDoubleSpinBox(GTextField *gtextField,
                                                   double min, double max,
                                                   double step, QWidget *parent)
    : QDoubleSpinBox(parent), _gtextfield(gtextField) {
  require::nonNull(gtextField, "_Internal_QDoubleSpinBox::constructor");
  setObjectName(QString::fromStdString("_Internal_QDoubleSpinBox_" +
                                       std::to_string(gtextField->getID())));
  setRange(min, max);
  setSingleStep(step);
}

void _Internal_QDoubleSpinBox::detach() { _gtextfield = nullptr; }
...
GEventQueue::GEventQueue() : _eventMask(0) {
  // empty
}
...
int GEventQueue::getEventMask() const { return _eventMask; }
...
bool GEventQueue::isEmpty() const { return _functionQueue.isEmpty(); }
...
void GEventQueue::setEventMask(int mask) { _eventMask = mask; }
 ...
 * @version 2018/12/28
 * - added methods for text selection, scrolling, cursor position, key/mouse
 * listeners
 * @version 2018/09/17
...
GBrowserPane::GBrowserPane(const std::string &url, QWidget *parent) {
  GThread::runOnQtGuiThread([this, url, parent]() {
    _iqtextbrowser =
        new _Internal_QTextBrowser(this, getInternalParent(parent));
  });
  if (!url.empty()) {
    readTextFromUrl(url);
  }
...
void GBrowserPane::clearText() {
  GThread::runOnQtGuiThread([this]() { _iqtextbrowser->clear(); });
}

std::string GBrowserPane::getContentType() const { return _contentType; }
...
std::string GBrowserPane::getPageUrl() const { return _pageUrl; }
...
std::string GBrowserPane::getType() const { return "GBrowserPane"; }

QWidget *GBrowserPane::getWidget() const {
  return static_cast<QWidget *>(_iqtextbrowser);
}

bool GBrowserPane::isEditable() const { return !_iqtextbrowser->isReadOnly(); }
...
void GBrowserPane::removeLinkListener() { removeEventListener("linkclick"); }
...
void GBrowserPane::selectAll() {
  GThread::runOnQtGuiThread([this]() { _iqtextbrowser->selectAll(); });
}
...
void GBrowserPane::setCursorPosition(int index, bool keepAnchor) {
  ...
  GThread::runOnQtGuiThread([this, index, keepAnchor]() {
    QTextCursor cursor(_iqtextbrowser->textCursor());
    cursor.setPosition(index, keepAnchor ? QTextCursor::KeepAnchor
...
void GBrowserPane::setEditable(bool value) {
  GThread::runOnQtGuiThread(
      [this, value]() { _iqtextbrowser->setReadOnly(!value); });
}

void GBrowserPane::setMouseListener(GEventListener func) {
  setEventListeners({"mousepress", "mouserelease"}, func);
}
...
void GBrowserPane::setLineWrap(bool wrap) {
  GThread::runOnQtGuiThread([this, wrap]() {
    _iqtextbrowser->setLineWrapMode(wrap ? QTextEdit::WidgetWidth
                                         : QTextEdit::NoWrap);
  });
}
...
_Internal_QTextBrowser::_Internal_QTextBrowser(GBrowserPane *gbrowserpane,
                                               QWidget *parent)
    : QTextBrowser(parent), _gbrowserpane(gbrowserpane) {
  require::nonNull(gbrowserpane, "_Internal_QTextBrowser::constructor");
  setObjectName(QString::fromStdString("_Internal_QTextBrowser_" +
                                       std::to_string(gbrowserpane->getID())));
  setFocusPolicy(Qt::StrongFocus);
}

void _Internal_QTextBrowser::detach() { _gbrowserpane = nullptr; }
...
GInputPanel::GInputPanel() : _loaded(false) {
  // empty
}

void GInputPanel::addButton(const std::string & /*text*/,
...
bool GInputPanel::isLoaded() const { return _loaded; }
...
void GInputPanel::load(const std::string &xmlFilename) {
  ...
  _loaded = true;
  rapidxml::xml_node<> *doc =
      xmlutils::openXmlDocument(xmlFilename, "inputpanel");
  for (rapidxml::xml_node<> *category :
       xmlutils::getChildNodes(doc, "category")) {
    std::string categoryName = xmlutils::getAttribute(category, "name");
    addCategory(categoryName);
    for (rapidxml::xml_node<> *button :
         xmlutils::getChildNodes(category, "button")) {
...
void GFont::boldFont(GInteractor *interactor) {
  ...
  QFont newFont =
      deriveQFont(toQFont(interactor->getFont()), /* weight */ QFont::Bold);
...
QFont GFont::deriveQFont(const QFont &font, const std::string &fontFamily,
                         QFont::Weight weight, int size) {
  return QFont(QString::fromStdString(fontFamily), size, weight,
               /* italic */ font.italic());
}

QFont GFont::deriveQFont(const std::string &font, QFont::Weight weight,
                         int size) {
  return deriveQFont(toQFont(font), weight, size);
}

QFont GFont::deriveQFont(const std::string &font, const std::string &fontFamily,
                         QFont::Weight weight, int size) {
...
QFont::StyleHint GFont::getStyleHint(const std::string &fontFamily) {
  ...
  if (fontFamilyStr == "arial" || fontFamilyStr == "helvetica" ||
      fontFamilyStr == "sansserif" || fontFamilyStr == "sans-serif") {
    return QFont::Helvetica;
  } else if (fontFamilyStr == "serif" || fontFamilyStr == "times" ||
             fontFamilyStr == "times roman" ||
             fontFamilyStr == "times new roman") {
    return QFont::Times;
  } else if (fontFamilyStr == "courier" || fontFamilyStr == "courier new" ||
             fontFamilyStr == "consolas" || fontFamilyStr == "monospace" ||
             fontFamilyStr == "monospaced" ||
             endsWith(fontFamilyStr, " mono")) {
    return QFont::Monospace;
  } else if (fontFamilyStr == "comic sans" ||
             fontFamilyStr == "comic sans ms" || fontFamilyStr == "fantasy") {
...
void GFont::italicFont(GInteractor *interactor) {
  ...
  QFont newFont(oldFont.family(), oldFont.pointSize(), oldFont.weight(),
                /* italic */ true);
  interactor->setFont(newFont);
}
...
QFont GFont::toQFont(const std::string &fontString) {
  ...
  QFont font = QFont(QString::fromStdString(fontFamily), fontSize,
                     fontBold ? QFont::Bold : QFont::Normal, fontItalic);
...
double GDimension::getWidth() const { return _width; }

double GDimension::getHeight() const { return _height; }
...
bool operator>(const GDimension &d1, const GDimension &d2) { return d2 < d1; }
...
HorizontalAlignment toHorizontalAlignment(const std::string &alignmentStr) {
  ...
  if (stringContains(alignLC, "left") || stringContains(alignLC, "west")) {
    return ALIGN_LEFT;
             ...
             stringContains(alignLC, "east")) {
    return ALIGN_RIGHT;
  } else if (stringContains(alignLC, "stretch") ||
             stringContains(alignLC, "justify")) {
...
double GPoint::getX() const { return _x; }

double GPoint::getY() const { return _y; }
...
bool operator==(const GPoint &p1, const GPoint &p2) {
  return floatingPointEqual(p1._x, p2._x) && floatingPointEqual(p1._y, p2._y);
}

bool operator!=(const GPoint &p1, const GPoint &p2) { return !(p1 == p2); }
...
bool operator>(const GPoint &p1, const GPoint &p2) { return p2 < p1; }
...
int hashCode(const GPoint &pt) { return hashCode(pt._x, pt._y); }
...
bool GRectangle::contains(const GRectangle &rect) const {
  return contains(rect.getX(), rect.getY()) &&
         contains(rect.getX() + rect.getWidth() - 1,
                  rect.getY() + rect.getHeight() - 1);
}

GRectangle GRectangle::enlargedBy(double amount) {
  return GRectangle(_x - amount, _y - amount, _width + 2 * amount,
                    _height + 2 * amount);
}

double GRectangle::getX() const { return _x; }

double GRectangle::getY() const { return _y; }

double GRectangle::getWidth() const { return _width; }

double GRectangle::getHeight() const { return _height; }

bool GRectangle::intersects(const GRectangle &other) const {
  // check for non-intersecting x coordinates
  return !(
      getX() + getWidth() < other.getX()          // I am entirely left of him
...
bool GRectangle::isEmpty() const { return _width <= 0 || _height <= 0; }
...
bool operator==(const GRectangle &r1, const GRectangle &r2) {
  return floatingPointEqual(r1._x, r2._x) && floatingPointEqual(r1._y, r2._y) &&
...
bool operator<(const GRectangle &r1, const GRectangle &r2) {
  return stanfordcpplib::collections::compareTo(r1._x, r2._x, r1._y, r2._y,
                                                r1._width, r2._width,
                                                r1._height, r2._height) < 0;
}
...
bool operator>(const GRectangle &r1, const GRectangle &r2) { return r2 < r1; }
...
/*static*/ const bool GConsoleWindow::GConsoleWindow::ALLOW_RICH_INPUT_EDITING =
    true;
...
/*static*/ const std::string GConsoleWindow::CONFIG_FILE_NAME =
    "spl-jar-settings.txt";
...
/*static*/ const std::string GConsoleWindow::DEFAULT_ERROR_COLOR_DARK_MODE =
    "#f47862";
/*static*/ const std::string GConsoleWindow::DEFAULT_USER_INPUT_COLOR =
    "#0000cc";
/*static*/ const std::string
    GConsoleWindow::DEFAULT_USER_INPUT_COLOR_DARK_MODE = "#2c90e5";
/*static*/ GConsoleWindow *GConsoleWindow::_instance = nullptr;
/*static*/ bool GConsoleWindow::_consoleEnabled = false;

/*static*/ bool GConsoleWindow::consoleEnabled() { return _consoleEnabled; }

/*static*/ std::string GConsoleWindow::getDefaultFont() {
  if (OS::isMac()) {
    // for some reason, using "Monospace" doesn't work for me on Mac testing
    return "Courier New-" + std::to_string(DEFAULT_FONT_SIZE + 1) +
           (DEFAULT_FONT_WEIGHT.empty() ? "" : ("-" + DEFAULT_FONT_WEIGHT));
  } else {
    return DEFAULT_FONT_FAMILY + "-" + std::to_string(DEFAULT_FONT_SIZE) +
           (DEFAULT_FONT_WEIGHT.empty() ? "" : ("-" + DEFAULT_FONT_WEIGHT));
  }
}
...
/*static*/ bool GConsoleWindow::isInitialized() { return _instance != nullptr; }
...
GConsoleWindow::GConsoleWindow()
    : GWindow(/* visible */ false), _textArea(nullptr), _clearEnabled(true),
      _echo(false), _locationSaved(false), _locked(false), _promptActive(false),
      _shutdown(false), _commandHistoryIndex(-1), _errorColor(""),
      _outputColor(""), _userInputColor(""), _inputBuffer(""),
      _lastSaveFileName(""), _cinout_new_buf(nullptr), _cerr_new_buf(nullptr),
      _cin_old_buf(nullptr), _cout_old_buf(nullptr), _cerr_old_buf(nullptr) {
  _initMenuBar();
  _initWidgets();
  _initStreams();
  loadConfiguration();
...
/*static*/ Map<std::string, QPixmap>
GConsoleWindow::unpackImageStrip(const std::string &imageStripFileName,
...
GConsoleWindow::unpackImageStrip(const std::string &imageStripFileName,
  ...
  if (fileExists(imageStripFileName)) {
    ...
    for (int i = 0; i < imageFiles.size(); i++) {
      ...
      QPixmap pixcopy =
          pixmap.copy(iconOffsetX, 0, static_cast<int>(imageSize.getWidth()),
                      static_cast<int>(imageSize.getHeight()));
...
void GConsoleWindow::_initMenuBar() {
  ...
  Vector<std::string> IMAGES{"save.gif",
                             "save_as.gif",
                             "print.gif",
                             "script.gif",
                             "compare_output.gif",
  ...
  Map<std::string, QPixmap> imageMap =
      unpackImageStrip(ICON_STRIP_FILE, IMAGES, ICON_SIZE);

  // File menu
  addMenu("&File");
  addMenuItem("File", "&Save", imageMap["save.gif"], [this]() {
    this->save();
  })->setShortcut(QKeySequence::Save);

  addMenuItem("File", "Save &As...", imageMap["save_as.gif"], [this]() {
    this->saveAs();
  })->setShortcut(QKeySequence::SaveAs);
  addMenuSeparator("File");

  addMenuItem("File", "&Print", imageMap["print.gif"], [this]() {
  ...
  addMenuItem("File", "&Quit", imageMap["quit.gif"], [this]() {
    this->close(); /* TODO: exit app */
  ...
  // Edit menu
  addMenu("&Edit");
  addMenuItem("Edit", "Cu&t", imageMap["cut.gif"], [this]() {
    this->clipboardCut();
  ...
  addMenuItem("Edit", "&Copy", imageMap["copy.gif"], [this]() {
    this->clipboardCopy();
  ...
  addMenuItem("Edit", "&Paste", imageMap["paste.gif"], [this]() {
    this->clipboardPaste();
  ...
  addMenuItem("Edit", "Select &All", imageMap["select_all.gif"], [this]() {
    this->selectAll();
  ...
  addMenuItem("Options", "&Background Color...",
              imageMap["background_color.gif"],
  ...
  // Help menu
  addMenu("&Help");
  addMenuItem("Help", "&About...", imageMap["about.gif"], [this]() {
...
void GConsoleWindow::_initStreams() {
  ...
  _cerr_new_buf =
      new stanfordcpplib::qtgui::ConsoleStreambufQt(/* isStderr */ true);
...
void GConsoleWindow::_initWidgets() {
  ...
  _textArea->setMouseListener([](GEvent event) {
    ...
    if (event.getButton() > 1 || event.getEventType() == MOUSE_RELEASED) {
      event.ignore();
    }
  });
...
void GConsoleWindow::checkForUpdates() {
  GThread::runInNewThreadAsync(
      [this]() {
        static const std::string CPP_ZIP_VERSION_URL =
            version::getCppLibraryDocsUrl() + "CURRENTVERSION_CPPLIB.txt";
        std::string currentVersion = version::getCppLibraryVersion();

        GDownloader downloader;
        std::string latestVersion =
            trim(downloader.downloadAsString(CPP_ZIP_VERSION_URL));

        if (latestVersion.empty()) {
          GOptionPane::showMessageDialog(
              /* parent  */ getWidget(),
              /* message */
              "Unable to look up latest library version from web.",
              /* title   */ "Network error",
        ...
        if (currentVersion >= latestVersion) {
          ...
          message = "<html>There is an updated version of the Stanford "
                    "libraries available.\n\n"
                    "This project's library version: " +
                    currentVersion +
                    "\n"
                    ...
                    "\n\n"
                    "Go to <a href=\"" +
                    version::getCppLibraryDocsUrl() + "\">" +
                    version::getCppLibraryDocsUrl() +
                    "</a> to get the new version.</html>";
        }
        GOptionPane::showMessageDialog(
            /* parent  */ getWidget(),
            /* message */ message);
...
void GConsoleWindow::clipboardCut() {
  ...
  // if selection is entirely within the user input area, cut out of user input
  // area
  ...
  if (selectionEnd > selectionStart && selectionStart >= userInputStart &&
    ...
    if (frag.isValid()) {
      std::string selectedText = _textArea->getSelectedText();
      QTextEdit *textArea =
          static_cast<QTextEdit *>(this->_textArea->getWidget());
...
void GConsoleWindow::compareOutput(const std::string &filename) {
  ...
  GDiffGui::showDialog("Expected Output", expectedOutput, "Your Output",
                       studentOutput,
...
std::string GConsoleWindow::getColor() const { return getOutputColor(); }
...
std::string GConsoleWindow::getErrorColor() const {
  if (!_errorColor.empty()) {
    ...
    return GWindow::isDarkMode() ? DEFAULT_ERROR_COLOR_DARK_MODE
                                 : DEFAULT_ERROR_COLOR;
  }
}

std::string GConsoleWindow::getFont() const { return _textArea->getFont(); }

std::string GConsoleWindow::getForeground() const { return getOutputColor(); }
...
std::string GConsoleWindow::getOutputColor() const {
  return _outputColor.empty() ? GWindow::getDefaultInteractorTextColor()
                              : _outputColor;
}
...
std::string GConsoleWindow::getUserInputColor() const {
  if (!_userInputColor.empty()) {
    ...
    return GWindow::isDarkMode() ? DEFAULT_USER_INPUT_COLOR_DARK_MODE
                                 : DEFAULT_USER_INPUT_COLOR;
  }
}
...
bool GConsoleWindow::hasInputScript() const { return !_inputScript.isEmpty(); }

bool GConsoleWindow::isClearEnabled() const { return _clearEnabled; }

bool GConsoleWindow::isCursorInUserInputArea() const {
  int cursorPosition = _textArea->getCursorPosition();
  int userInputStart = getUserInputStart();
  int userInputEnd = getUserInputEnd();
  return _promptActive && userInputStart <= cursorPosition &&
         cursorPosition <= userInputEnd;
}

bool GConsoleWindow::isEcho() const { return _echo; }

bool GConsoleWindow::isLocationSaved() const { return _locationSaved; }

bool GConsoleWindow::isLocked() const { return _locked; }
...
bool GConsoleWindow::isSelectionInUserInputArea() const {
  ...
  return userInputStart >= 0 && userInputEnd >= 0 &&
         selectionStart >= userInputStart && selectionEnd <= userInputEnd;
}
    ...
    for (std::string filename : listDirectory(dir)) {
      ...
      if (inputFile.empty() &&
        ...
        inputFile = filename;
                 ...
                 stringContains(filename,
                                "expected-output-" + std::to_string(number)) &&
                 endsWith(filename, ".txt")) {
...
void GConsoleWindow::print(const std::string &str, bool isStdErr) {
  ...
  GThread::runOnQtGuiThread([this, strToPrint, isStdErr]() {
    ...
    this->_textArea->appendFormattedText(
        strToPrint, isStdErr ? getErrorColor() : getOutputColor());
...
void GConsoleWindow::println(bool isStdErr) { print("\n", isStdErr); }
...
void GConsoleWindow::processKeyPress(GEvent event) {
      ...
      if (_promptActive) {
        ...
        if (start >= 0) {
          _textArea->setCursorPosition(
              start,
              /* keepAnchor */ event.isShiftKeyDown() &&
                  isCursorInUserInputArea());
        } else {
          _textArea->moveCursorToEnd();
        }
      }
  ...
  case GEvent::MENU_KEY: {
    // various other control/modifier keys: let OS have the event (don't call
    // ignore())
    break;
  }
...
void GConsoleWindow::processBackspace(int key) {
  ...
  bool isBackspace =
      key == GEvent::BACKSPACE_KEY /* TODO: or computer is Mac */;
  ...
  if (!_inputBuffer.empty()) {
    ...
    if (frag.isValid()) {
      ...
      int userInputIndexMax =
          frag.position() + frag.length() - (isBackspace ? 0 : 1);

      if (oldCursorPosition >= userInputIndexMin &&
          oldCursorPosition < userInputIndexMax) {
        // cursor is inside the user input fragment;
        // figure out which character it's on so we can delete it
        indexToDelete =
            oldCursorPosition - frag.position() - (isBackspace ? 1 : 0);
      } else {
...
void GConsoleWindow::processCommandHistory(int delta) {
  ...
  _commandHistoryIndex =
      std::min(_commandHistoryIndex, _inputCommandHistory.size());
...
void GConsoleWindow::processEof() {
  ...
  // only set EOF if input buffer is empty; this is the behavior on most *nix
  // consoles
  if (_inputBuffer.empty()) {
...
void GConsoleWindow::processUserInputKey(int key) {
  ...
  if (key != '\0' && isprint(key)) {
      ...
      if (frag.isValid()) {
        ...
        if (indexToInsert == 0) {
          ...
          cursor.insertText(QString::fromStdString(
              keyStr +
              _inputBuffer.substr(0, 1))); // insert new char + old first char
    ...
    if (!inserted) {
      ...
      this->_textArea->appendFormattedText(keyStr, getUserInputColor(),
                                           "*-*-Bold");
    }
...
void GConsoleWindow::save() { saveAs(_lastSaveFileName); }
...
void GConsoleWindow::saveAs(const std::string &filename) {
  ...
  if (filename.empty()) {
    filenameToUse = GFileChooser::showSaveDialog(
        ...
        /* title */ "", getHead(_lastSaveFileName));
  } else {
...
void GConsoleWindow::selectAll() { _textArea->selectAll(); }
...
void GConsoleWindow::setColor(int color) { setOutputColor(color); }
...
void GConsoleWindow::setForeground(int color) { setOutputColor(color); }
...
void GConsoleWindow::setLocked(bool locked) { _locked = locked; }
...
void GConsoleWindow::showAboutDialog() {
  ...
  static const std::string ABOUT_MESSAGE =
      "<html><p>"
      "Stanford C++ Library version <b>" +
      version::getCppLibraryVersion() +
      "</b><br>\n"
      "<br>\n"
      "Libraries originally written by <b>Eric Roberts</b>,<br>\n"
      "with assistance from Julie Zelenski, Keith Schwarz, et al.<br>\n"
      "This version of the library is unofficially maintained by <b>Marty "
      "Stepp</b>.<br>\n"
      "<br>\n"
      "See <a href=\"" +
      version::getCppLibraryDocsUrl() + "\">" +
...
void GConsoleWindow::showColorDialog(bool background) {
  std::string color = GColorChooser::showDialog(
      ...
      /* title */ "",
      /* initial */
      background ? _textArea->getBackground() : _textArea->getForeground());
  if (!color.empty()) {
    if (background) {
      setBackground(color);
    } else {
...
GEvent::GEvent(EventClass eventClass, EventType eventType,
               const std::string &eventName, GObservable *source)
    : _actionCommand(""), _class(eventClass), _keyChar('\0'), _keyCode(0),
      _modifiers(0), _name(eventName), _source(source),
      _time(getCurrentTimeMS()), _type(eventType), _x(0), _y(0), _row(0),
      _col(0), _internalQtEvent(nullptr) {
  // empty
}

GEvent::~GEvent() { _source = nullptr; }
...
std::string GEvent::classToString(EventClass eventClass) {
  ...
  case CHANGE_EVENT:
    return "CHANGE_EVENT";
  case HYPERLINK_EVENT:
    return "HYPERLINK_EVENT";
  case SCROLL_EVENT:
    return "SCROLL_EVENT";
  default:
    return "UNKNOWN";
...
std::string GEvent::typeToString(EventType eventType) {
  ...
  case TABLE_EDIT_BEGIN:
    return "TABLE_EDIT_BEGIN";
  case TABLE_REPLACE_BEGIN:
    return "TABLE_REPLACE_BEGIN";
  case TABLE_EDIT_CANCEL:
    return "TABLE_EDIT_CANCEL";
  case TABLE_CUT:
    return "TABLE_CUT";
  case TABLE_COPY:
    return "TABLE_COPY";
  ...
  case SCROLL_SCROLLED:
    return "SCROLL_SCROLLED";
  default:
    return "UNKNOWN";
  }
}

std::string GEvent::getActionCommand() const { return _actionCommand; }

int GEvent::getButton() const { return _button; }

EventClass GEvent::getClass() const { return _class; }

int GEvent::getColumn() const { return _col; }
...
EventClass GEvent::getEventClass() const { return _class; }

EventType GEvent::getEventType() const { return _type; }

char GEvent::getKeyChar() const { return _keyChar; }

int GEvent::getKeyCode() const { return _keyCode; }

int GEvent::getModifiers() const { return _modifiers; }

std::string GEvent::getName() const { return _name; }

std::string GEvent::getRequestURL() const { return _requestUrl; }

GInteractor *GEvent::getInteractor() const {
  return static_cast<GInteractor *>(_source);
}

QEvent *GEvent::getInternalEvent() const { return _internalQtEvent; }

GPoint GEvent::getLocation() const { return GPoint(getX(), getY()); }

int GEvent::getRow() const { return _row; }

GObservable *GEvent::getSource() const { return _source; }

long GEvent::getTime() const { return _time; }

EventType GEvent::getType() const { return _type; }

double GEvent::getX() const { return _x; }

double GEvent::getY() const { return _y; }
...
bool GEvent::isAltKeyDown() const { return (_modifiers & ALT_DOWN) != 0; }

bool GEvent::isCtrlKeyDown() const { return (_modifiers & CTRL_DOWN) != 0; }
...
bool GEvent::isLeftClick() const { return !isMiddleClick() && !isRightClick(); }
...
bool GEvent::isMetaKeyDown() const { return (_modifiers & META_DOWN) != 0; }

bool GEvent::isShiftKeyDown() const { return (_modifiers & SHIFT_DOWN) != 0; }
...
std::string GEvent::keyCodeToString(int keyCode) {
  ...
  case '\\':
    return "\\";
  default:
    return charToString((char)keyCode);
  }
}
...
void GEvent::setButton(int button) { _button = button; }

void GEvent::setInternalEvent(QEvent *event) { _internalQtEvent = event; }
...
void GEvent::setSource(GObservable *source) { _source = source; }

void GEvent::setX(double x) { _x = x; }

void GEvent::setY(double y) { _y = y; }
 ...
 * from
 * https://stackoverflow.com/questions/2427103/qt-how-to-force-a-hidden-widget-to-calculate-its-layout
 */
...
GBorderLayout::GBorderLayout(int spacing) { setSpacing(spacing); }
...
bool GBorderLayout::hasHeightForWidth() const { return false; }

int GBorderLayout::count() const { return list.size(); }
...
QSize GBorderLayout::minimumSize() const { return calculateSize(MinimumSize); }
...
void GBorderLayout::setGeometry(const QRect &rect) {
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    if (position == GLayout::North) {
      QRect geom(rect.x(), northHeight, rect.width(),
                 item->sizeHint().height());
      item->setGeometry(geom);
      northHeight += item->geometry().height() + spacing();
    } else if (position == GLayout::South) {
      QRect geom(item->geometry().x(), item->geometry().y(), rect.width(),
                 item->sizeHint().height());
      item->setGeometry(geom);

      southHeight += item->geometry().height() + spacing();
      geom = QRect(rect.x(), rect.y() + rect.height() - southHeight + spacing(),
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    if (position == GLayout::West) {
      QRect geom(rect.x() + westWidth, northHeight, item->sizeHint().width(),
                 centerHeight);
      item->setGeometry(geom);
      westWidth += item->geometry().width() + spacing();
    } else if (position == GLayout::East) {
      QRect geom(item->geometry().x(), item->geometry().y(),
                 item->sizeHint().width(), centerHeight);
      item->setGeometry(geom);

      eastWidth += item->geometry().width() + spacing();
      geom = QRect(rect.x() + rect.width() - eastWidth + spacing(), northHeight,
                   item->geometry().width(), item->geometry().height());
      item->setGeometry(geom);
    }
  }
  ...
  if (center) {
    QRect geom(westWidth, northHeight, rect.width() - eastWidth - westWidth,
               centerHeight);
    center->item->setGeometry(geom);
  }
}

QSize GBorderLayout::sizeHint() const { return calculateSize(SizeHint); }
...
QSize GBorderLayout::calculateSize(SizeType sizeType) const {
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    if (position == GLayout::North || position == GLayout::South ||
        position == GLayout::Center) {
      totalSize.rheight() += itemSize.height();
    }

    if (position == GLayout::West || position == GLayout::East ||
        position == GLayout::Center) {
...
int GCanvas::createArgbPixel(int alpha, int red, int green, int blue) {
  ...
  return (alpha << 24 & 0xff000000) | (red << 16 & 0xff0000) |
         (green << 8 & 0x00ff00) | (blue & 0x0000ff);
}
...
int GCanvas::getBlue(int rgb) { return rgb & 0x0000ff; }

int GCanvas::getGreen(int rgb) { return (rgb & 0x00ff00) >> 8; }

int GCanvas::getRed(int rgb) { return (rgb & 0xff0000) >> 16; }
...
GCanvas::GCanvas(QWidget *parent) : _backgroundImage(nullptr), _filename("") {
  init(/* width */ -1, /* height */ -1, /* background */ 0xffffff, parent);
}

GCanvas::GCanvas(const std::string &filename, QWidget *parent)
    : _backgroundImage(nullptr), _filename(filename) {
  init(/* width */ -1, /* height */ -1, /* background */ 0xffffff, parent);
  load(filename);
}

GCanvas::GCanvas(std::istream &source, QWidget *parent)
    : _backgroundImage(nullptr), _filename("std::istream data") {
  init(/* width */ -1, /* height */ -1, /* background */ 0xffffff, parent);
...
GCanvas::GCanvas(double width, double height, int rgbBackground,
                 QWidget *parent)
    : _backgroundImage(nullptr), _filename("") {
  init(width, height, rgbBackground, parent);
}

GCanvas::GCanvas(double width, double height, const std::string &rgbBackground,
                 QWidget *parent)
    : _backgroundImage(nullptr), _filename("") {
...
void GCanvas::init(double width, double height, int rgbBackground,
                   QWidget *parent) {
...
void GCanvas::clearPixels() {
  GThread::runOnQtGuiThread([this]() {
    ...
    if (_backgroundImage) {
      ...
      GThread::runOnQtGuiThread([this]() {
        if (GColor::hasAlpha(_backgroundColor)) {
          _backgroundImage->fill(
              static_cast<unsigned int>(_backgroundColorInt));
        } else {
          _backgroundImage->fill(
              static_cast<unsigned int>(_backgroundColorInt) | 0xff000000);
        }
      });
    }
    unlock();
...
int GCanvas::countDiffPixels(const GCanvas &image) const {
  ...
  for (int y = 0; y < hmin; y++) {
    for (int x = 0; x < wmin; x++) {
      int px1 =
          _backgroundImage->pixel(static_cast<int>(x), static_cast<int>(y)) &
          0x00ffffff;
      int px2 = image._backgroundImage->pixel(static_cast<int>(x),
                                              static_cast<int>(y)) &
                0x00ffffff;
      if (px1 != px2) {
        diffPxCount++;
...
int GCanvas::countDiffPixels(const GCanvas &image, int xmin, int ymin, int xmax,
                             int ymax) const {
  lockForReadConst();
  int w1 = static_cast<int>(getWidth());
  int h1 = static_cast<int>(getHeight());
  ...
  for (int y = ymin; y < ymax; y++) {
    for (int x = xmin; x < xmax; x++) {
      int px1 = x < w1 && y < h1
                    ? (_backgroundImage->pixel(static_cast<int>(x),
                                               static_cast<int>(y)) &
                       0x00ffffff)
                    : -1;
      int px2 = x < w2 && y < h2
                    ? (image._backgroundImage->pixel(static_cast<int>(x),
                                                     static_cast<int>(y)) &
                       0x00ffffff)
                    : -1;
      if (px1 != px2) {
...
void GCanvas::draw(GObject *gobj) {
  ...
  if (_backgroundImage && _backgroundImage->paintEngine()) {
    ...
    conditionalRepaintRegion(
        gobj->getBounds().enlargedBy((gobj->getLineWidth() + 1) / 2));
  }
}
...
void GCanvas::ensureBackgroundImage() {
  if (!_backgroundImage) {
    GThread::runOnQtGuiThread([this]() {
      lockForWrite();
      _backgroundImage =
          new QImage(static_cast<int>(getWidth()),
                     static_cast<int>(getHeight()), QImage::Format_ARGB32);
...
void GCanvas::fillRegion(double x, double y, double width, double height,
                         int rgb) {
  checkBounds("GCanvas::fillRegion", x, y, getWidth(), getHeight());
  checkBounds("GCanvas::fillRegion", x + width - 1, y + height - 1, getWidth(),
              getHeight());
...
void GCanvas::fromGrid(const Grid<int> &grid) {
  ...
  GThread::runOnQtGuiThread([this, &grid]() {
    ensureBackgroundImage();
    lockForWrite();
    for (int row = 0, width = grid.width(), height = grid.height();
         row < height; row++) {
...
std::string GCanvas::getFilename() const { return _filename; }

std::string GCanvas::getFont() const { return GDrawingSurface::getFont(); }

_Internal_QWidget *GCanvas::getInternalWidget() const { return _iqcanvas; }
...
std::string GCanvas::getType() const { return "GCanvas"; }

QWidget *GCanvas::getWidget() const {
  return static_cast<QWidget *>(_iqcanvas);
}

bool GCanvas::isAutoRepaint() const { return _gcompound.isAutoRepaint(); }
...
bool GCanvas::loadFromStream(std::istream &input) {
  ...
  GThread::runOnQtGuiThread([&, this]() {
    ensureBackgroundImage();
    lockForWrite();
    if (!_backgroundImage->loadFromData(
            reinterpret_cast<const uchar *>(bytes.data()), bytes.length())) {
      hasError = true;
      unlock();
      return;
    }
...
void GCanvas::notifyOfResize(double width, double height) {
  if (_backgroundImage) {
    GThread::runOnQtGuiThread([this, width, height]() {
      ...
      lockForWrite();
      QImage *newImage =
          new QImage(static_cast<int>(width), static_cast<int>(height),
  ...
  GThread::runOnQtGuiThread([this, filename]() {
    ...
    if (!_gcompound.isEmpty()) {
      ...
      QImage imageCopy = this->_backgroundImage->copy(
          0, 0, static_cast<int>(getWidth()), static_cast<int>(getHeight()));
...
void GCanvas::setBackground(int color) {
  ...
  if (_backgroundImage) {
    ...
    // if you draw some shapes on a canvas (with draw() variants) and then set
    // its background, the shapes will get wiped out. The lesson is, set the
    // background first before drawing stuff. Or add your shapes using add()
    // rather than draw() so they sit atop the background.
...
void GCanvas::setColor(int color) { GDrawingSurface::setColor(color); }

void GCanvas::setColor(const std::string &color) {
  setColor(GColor::convertColorToRGB(color));
}

void GCanvas::setFont(const QFont &font) { GDrawingSurface::setFont(font); }
...
void GCanvas::setPixel(double x, double y, int rgb) {
  require::inRange2D(x, y, getWidth(), getHeight(), "GCanvas::setPixel", "x",
                     "y");
  ...
  GThread::runOnQtGuiThread([this, x, y, rgb]() {
    ...
    _backgroundImage->setPixel(static_cast<int>(x), static_cast<int>(y),
                               static_cast<unsigned int>(argb));
    unlock();
    conditionalRepaintRegion(static_cast<int>(x), static_cast<int>(y),
                             /* width */ 1,
                             /* height */ 1);
  });
}
...
void GCanvas::setPixelARGB(double x, double y, int argb) {
  require::inRange2D(x, y, getWidth(), getHeight(), "GCanvas::setPixelARGB",
                     "x", "y");
...
void GCanvas::setPixels(const Grid<int> &pixels) {
  ...
  ensureBackgroundImage();
  if (pixels.width() != (int)getWidth() ||
      pixels.height() != (int)getHeight()) {
    // TODO
...
GImage *GCanvas::toGImage() const {
  ...
  //        QImage* copy = new QImage((int) that->getWidth(), (int)
  //        that->getHeight(), QImage::Format_ARGB32); *copy =
  //        backgroundImage->copy(); QPainter painter(copy);
  //        painter.setRenderHint(QPainter::Antialiasing,
  //        GObject::isAntiAliasing());
  //        painter.setRenderHint(QPainter::TextAntialiasing,
  //        GObject::isAntiAliasing()); that->_gcompound.draw(&painter);
  //        painter.end();
...
void GCanvas::toGrid(Grid<int> &grid) const {
  ...
  lockForReadConst();
  for (int row = 0, width = (int)getWidth(), height = (int)getHeight();
       row < height; row++) {
    for (int col = 0; col < width; col++) {
...
_Internal_QCanvas::_Internal_QCanvas(GCanvas *gcanvas, QWidget *parent)
    : QWidget(parent), _gcanvas(gcanvas) {
  require::nonNull(gcanvas, "_Internal_QCanvas::constructor");
  setObjectName(QString::fromStdString("_Internal_QCanvas_" +
                                       std::to_string(gcanvas->getID())));
...
void _Internal_QCanvas::detach() { _gcanvas = nullptr; }
...
GInteractor::GInteractor()
    : _actionCommand(""), _actionEventType("click"), _icon(""), _name(""),
      _id(-1), _container(nullptr), _lock(QReadWriteLock::Recursive) {
...
std::string GInteractor::getActionCommand() const { return _actionCommand; }

std::string GInteractor::getActionEventType() const { return _actionEventType; }
...
GContainer *GInteractor::getContainer() const { return _container; }
...
double GInteractor::getHeight() const { return getWidget()->height(); }

int GInteractor::getID() const { return _id; }

std::string GInteractor::getIcon() const { return _icon; }

GPoint GInteractor::getLocation() const { return GPoint(getX(), getY()); }
...
double GInteractor::getWidth() const { return getWidget()->width(); }

double GInteractor::getX() const { return getWidget()->x(); }

double GInteractor::getY() const { return getWidget()->y(); }
...
bool GInteractor::isEnabled() const { return getWidget()->isEnabled(); }

bool GInteractor::isVisible() const { return getWidget()->isVisible(); }

void GInteractor::lockForRead() { _lock.lockForRead(); }
...
void GInteractor::lockForWrite() { _lock.lockForWrite(); }
...
void GInteractor::removeClickListener() { removeEventListener("click"); }
...
void GInteractor::removeKeyListener() {
  removeEventListeners({"keypress", "keyrelease", "keytype"});
}

void GInteractor::removeMouseListener() {
  removeEventListeners({"click", "mousedrag", "mouseenter", "mouseexit",
                        "mousemove", "mousepress", "mouserelease",
                        "mousewheeldown", "mousewheelup"});
}

void GInteractor::requestFocus() {
  GThread::runOnQtGuiThread([this]() { getWidget()->setFocus(); });
}
...
void GInteractor::setBackground(int rgb) {
  GThread::runOnQtGuiThread([this, rgb]() {
    ...
    } else if (getType() == "GTextArea" || getType() == "GBrowserPane") {
      palette.setColor(QPalette::Base, QColor(rgb));
    }
...
void GInteractor::setColor(int rgb) { setForeground(rgb); }

void GInteractor::setColor(const std::string &color) { setForeground(color); }
...
void GInteractor::setContainer(GContainer *container) {
  ...
  if (!container) {
    ...
    GThread::runOnQtGuiThread([widget]() { widget->setParent(nullptr); });
    setVisible(false);
  }
}
...
void GInteractor::setEnabled(bool value) {
  GThread::runOnQtGuiThread(
      [this, value]() { getWidget()->setEnabled(value); });
}
...
void GInteractor::setForeground(const std::string &color) {
  if (GColor::hasAlpha(color)) {
    ...
    GThread::runOnQtGuiThread([this, argb]() {
      QPalette palette(getWidget()->palette());
      palette.setColor(getWidget()->foregroundRole(),
                       GColor::toQColorARGB(argb));
      // TODO: does not totally work for some widgets, e.g. GChooser popup menu
      getWidget()->setPalette(palette);
    });
  } else {
...
void GInteractor::setFont(const QFont &font) {
  GThread::runOnQtGuiThread([this, font]() { getWidget()->setFont(font); });
}
...
void GInteractor::setHeight(double height) {
  ...
  GThread::runOnQtGuiThread(
      [this, height]() { getWidget()->setFixedHeight((int)height); });
}
...
void GInteractor::setKeyListener(GEventListener func) {
  setEventListeners({"keypress", "keyrelease", "keytype"}, func);
}
...
void GInteractor::setMouseListener(GEventListener func) {
  setEventListeners({"click", "mousedrag", "mouseenter", "mouseexit",
                     "mousemove", "mousepress", "mouserelease",
                     "mousewheeldown", "mousewheelup"},
                    func);
}
...
void GInteractor::setVisible(bool visible) {
  ...
  if (!visible || _container) {
    GThread::runOnQtGuiThread(
        [this, visible]() { getWidget()->setVisible(visible); });
  }
}

void GInteractor::setWidth(double width) {
  GThread::runOnQtGuiThread(
      [this, width]() { getWidget()->setFixedWidth((int)width); });
}

void GInteractor::setX(double x) { setLocation(x, getY()); }

void GInteractor::setY(double y) { setLocation(getX(), y); }

void GInteractor::unlock() { _lock.unlock(); }
...
_Internal_QWidget::_Internal_QWidget()
    : _minimumSize(-1, -1), _preferredSize(-1, -1) {
  // empty
}

_Internal_QWidget::~_Internal_QWidget() { detach(); }
 ...
 * @version 2018/09/05
 * - refactored to use a border layout GContainer "content pane" for storing all
 * interactors
 * @version 2018/08/23
...
/*static*/ const std::string GWindow::DEFAULT_ICON_FILENAME =
    "splicon-large.png";

GWindow::GWindow(bool visible)
    : _iqmainwindow(nullptr), _contentPane(nullptr), _canvas(nullptr),
      _resizable(true), _closeOperation(GWindow::CLOSE_DISPOSE),
      _toolbar(nullptr) {
...
void GWindow::_init(double width, double height, bool visible) {
  require::nonNegative2D(width, height, "GWindow::constructor", "width",
                         "height");
...
QMenu *GWindow::addMenu(const std::string &menu) {
  ...
  GThread::runOnQtGuiThread([this, menu, &qmenu]() {
    ...
    std::string menuKey =
        toLowerCase(stringReplace(stringReplace(menu, "/", ""), "&", ""));
    _menuMap[menuKey] = qmenu;
  });
  return qmenu;
}

QAction *GWindow::addMenuItem(const std::string &menu, const std::string &item,
...
QAction *GWindow::addMenuItem(const std::string &menu, const std::string &item,
  ...
  GThread::runOnQtGuiThread(
      [this, menu, item, icon, func, menuKey, menuItemKey, &action]() {
        ...
        _iqmainwindow->connect(action, &QAction::triggered, _iqmainwindow,
                               [func]() { func(); });
...
QAction *GWindow::addMenuItemCheckBox(const std::string &menu,
                                      const std::string &item, bool checked,
                                      const std::string &icon) {
  GEventListenerVoid func = [this, menu, item]() {
    this->_iqmainwindow->handleMenuAction(menu, item);
  };
...
QAction *GWindow::addMenuItemCheckBox(const std::string &menu,
  ...
  GThread::runOnQtGuiThread(
      [this, menu, item, icon, checked, func, menuKey, &action]() {
        QMenu *qmenu = _menuMap[menuKey];
        action = qmenu->addAction(QString::fromStdString(item));
...
QMenu *GWindow::addSubMenu(const std::string &menu,
  ...
  GThread::runOnQtGuiThread([this, menu, menuKey, submenu, &qsubmenu]() {
    ...
    std::string subMenuKey =
        menuKey + "/" + toLowerCase(stringReplace(submenu, "&", ""));
...
void GWindow::addToRegion(GInteractor *interactor, Region region) {
  ...
  _contentPane->addToRegion(interactor,
                            static_cast<GContainer::Region>(region));
}
...
QAction *GWindow::addToolbarItem(const std::string &item, const QIcon &icon,
                                 GEventListenerVoid func) {
  if (!_toolbar) {
    addToolbar();
  }
...
QAction *GWindow::addToolbarItem(const std::string &item, const QPixmap &icon,
                                 GEventListenerVoid func) {
  if (!_toolbar) {
    addToolbar();
  }
...
QAction *GWindow::addToolbarSeparator() {
  ...
  QAction *action = nullptr;
  GThread::runOnQtGuiThread(
      [this, &action]() { action = _toolbar->addSeparator(); });
  return action;
}
...
void GWindow::clear() {
  ...
  //    bool hasCanvas = _canvas && _contentPane->regionContains(_canvas,
  //    GContainer::REGION_CENTER);
...
void GWindow::clearToolbarItems() {
  ...
  GThread::runOnQtGuiThread([this]() { _toolbar->clear(); });
}
...
/*static*/ std::string
GWindow::chooseLightDarkModeColor(const std::string &lightColor,
                                  const std::string &darkColor) {
  return isDarkMode() ? darkColor : lightColor;
}

/*static*/ int GWindow::chooseLightDarkModeColorInt(int lightColor,
                                                    int darkColor) {
  return isDarkMode() ? darkColor : lightColor;
}

void GWindow::close() {
  GThread::runOnQtGuiThread([this]() { _iqmainwindow->close(); });
}

void GWindow::compareToImage(const std::string &filename,
                             bool /*ignoreWindowSize*/) const {
  ensureForwardTargetConstHack();
  GCanvas *fileCanvas = new GCanvas(filename);
  GDiffImage::showDialog("expected output", fileCanvas, "your output", _canvas);
...
/* static */ QMainWindow *GWindow::getLastWindow() { return _lastWindow; }

double GWindow::getHeight() const { return _iqmainwindow->height(); }
...
/*static*/ GDimension GWindow::getScreenSize() {
  QRect rec;
  GThread::runOnQtGuiThread(
      [&rec]() { rec = QApplication::desktop()->availableGeometry(); });
...
std::string GWindow::getType() const { return "GWindow"; }

QWidget *GWindow::getWidget() const {
  return static_cast<QWidget *>(_iqmainwindow);
}

double GWindow::getWidth() const { return _iqmainwindow->width(); }

double GWindow::getX() const { return _iqmainwindow->x(); }

double GWindow::getY() const { return _iqmainwindow->y(); }

bool GWindow::hasToolbar() const { return _toolbar != nullptr; }

void GWindow::hide() { setVisible(false); }
...
bool GWindow::isOpen() const { return isVisible(); }

bool GWindow::isRepaintImmediately() const {
  return _canvas && _canvas->isRepaintImmediately();
}

bool GWindow::isResizable() const { return _resizable; }

bool GWindow::isVisible() const { return _iqmainwindow->isVisible(); }
...
void GWindow::maximize() {
  GThread::runOnQtGuiThread(
      [this]() { _iqmainwindow->setWindowState(Qt::WindowMaximized); });
}

void GWindow::minimize() {
  GThread::runOnQtGuiThread(
      [this]() { _iqmainwindow->setWindowState(Qt::WindowMinimized); });
}

void GWindow::pack() { setSize(getPreferredSize()); }
...
void GWindow::remove(GInteractor &interactor) { remove(&interactor); }
...
void GWindow::removeFromRegion(GInteractor *interactor, Region region) {
  ...
  _contentPane->removeFromRegion(interactor,
                                 static_cast<GContainer::Region>(region));
}

void GWindow::removeFromRegion(GInteractor *interactor,
...
void GWindow::removeMenuListener() { removeEventListener("actionMenu"); }
...
void GWindow::removeTimerListener() { removeEventListener("timer"); }
...
void GWindow::removeWindowListener() {
  removeEventListeners({"close", "closing", "maximize", "minimize", "open",
                        "resize", "restore"});
}
...
void GWindow::restore() {
  GThread::runOnQtGuiThread(
      [this]() { _iqmainwindow->setWindowState(Qt::WindowActive); });
}
...
void GWindow::setBackground(int color) {
  ...
  GThread::runOnQtGuiThread(
      [this, color]() { GForwardDrawingSurface::setBackground(color); });
}
...
void GWindow::setCanvasSize(double width, double height) {
  require::nonNegative2D(width, height, "GWindow::setCanvasSize", "width",
                         "height");
  ensureForwardTarget();
...
void GWindow::setExitOnClose(bool exitOnClose) {
  if (exitOnClose) {
    ...
    setCloseOperation(_closeOperation == GWindow::CLOSE_EXIT
                          ? GWindow::CLOSE_HIDE
                          : _closeOperation);
  }
}

void GWindow::setHeight(double height) { setSize(getWidth(), height); }
...
void GWindow::setLocation(const GPoint &p) { setLocation(p.getX(), p.getY()); }

void GWindow::setLocation(const Point &p) { setLocation(p.getX(), p.getY()); }

void GWindow::setMenuItemEnabled(const std::string &menu,
  ...
  if (!_menuMap.containsKey(menuKey)) {
    ...
    error("GWindow::setMenuItemEnabled: menu item \"" + item +
          "\" does not exist");
  }

  QAction *action = _menuActionMap[menuItemKey];
  GThread::runOnQtGuiThread(
      [action, enabled]() { action->setEnabled(enabled); });
}
...
void GWindow::setRegionAlignment(Region region, HorizontalAlignment halign) {
  _contentPane->setRegionAlignment(static_cast<GContainer::Region>(region),
                                   halign);
}
...
void GWindow::setRegionAlignment(const std::string &region,
...
void GWindow::setRegionHorizontalAlignment(Region region,
                                           HorizontalAlignment halign) {
  _contentPane->setRegionHorizontalAlignment(
      static_cast<GContainer::Region>(region), halign);
}

void GWindow::setRegionHorizontalAlignment(const std::string &region,
                                           const std::string &halign) {
  _contentPane->setRegionHorizontalAlignment(region, halign);
}

void GWindow::setRegionVerticalAlignment(Region region,
                                         VerticalAlignment valign) {
  _contentPane->setRegionVerticalAlignment(
      static_cast<GContainer::Region>(region), valign);
}

void GWindow::setRegionVerticalAlignment(const std::string &region,
                                         const std::string &valign) {
  ...
  GThread::runOnQtGuiThread([this, resizable]() {
    if (resizable) {
      if (!_resizable) {
        ...
        _iqmainwindow->setSizePolicy(QSizePolicy::Expanding,
...
void GWindow::setSize(double width, double height) {
  ...
  GThread::runOnQtGuiThread([this, width, height]() {
    if (isResizable()) {
      _iqmainwindow->resize(static_cast<int>(width), static_cast<int>(height));
    } else {
...
void GWindow::setTimerListener(double ms, GEventListener func) {
  ...
  GThread::runOnQtGuiThread([this, ms]() { _iqmainwindow->timerStart(ms); });
}
...
void GWindow::setVisible(bool visible) {
  GThread::runOnQtGuiThread(
      [this, visible]() { _iqmainwindow->setVisible(visible); });
}

void GWindow::setWidth(double width) { setSize(width, getHeight()); }
...
void GWindow::setWindowListener(GEventListener func) {
  setEventListeners(
      {"close", "closing", "maximize", "minimize", "open", "resize", "restore"},
      func);
}
...
void GWindow::setWindowTitle(const std::string &title) { setTitle(title); }

void GWindow::setX(double x) { setLocation(x, getY()); }

void GWindow::setY(double y) { setLocation(getX(), y); }

void GWindow::show() { setVisible(true); }
...
void GWindow::toBack() {
  GThread::runOnQtGuiThread([this]() { _iqmainwindow->lower(); });
}
...
void exitGraphics() { QtGui::instance()->exitGraphics(); }

double getScreenHeight() { return GWindow::getScreenHeight(); }

GDimension getScreenSize() { return GWindow::getScreenSize(); }

double getScreenWidth() { return GWindow::getScreenWidth(); }
...
_Internal_QMainWindow::_Internal_QMainWindow(GWindow *gwindow, QWidget *parent)
    : QMainWindow(parent), _gwindow(gwindow) {
...
void _Internal_QMainWindow::handleMenuAction(const std::string &menu,
...
GObservable::GObservable() : _eventsEnabled(true) {
  // empty
}
...
void GObservable::fireGEvent(QEvent *event, EventType eventType,
...
void GObservable::fireGEvent(QCloseEvent *event, EventType eventType,
                             const std::string &eventName) {
  require::nonNull(event, "GObservable::fireGEvent", "event");
  GEvent windowEvent(
      /* class  */ WINDOW_EVENT,
...
void GObservable::fireGEvent(QKeyEvent *event, EventType eventType,
                             const std::string &eventName) {
  require::nonNull(event, "GObservable::fireGEvent", "event");
  GEvent keyEvent(
      /* class  */ KEY_EVENT,
...
void GObservable::fireGEvent(QMouseEvent *event, EventType eventType,
                             const std::string &eventName,
...
void GObservable::fireGEvent(QResizeEvent *event, EventType /* eventType */,
                             const std::string &eventName) {
  require::nonNull(event, "GObservable::fireGEvent", "event");
  GEvent windowEvent(
      /* class  */ WINDOW_EVENT,
...
void GObservable::fireGEvent(QTimerEvent *event, EventType /* eventType */,
                             const std::string & /* eventName */) {
  require::nonNull(event, "GObservable::fireGEvent", "event");
  GEvent timerEvent(
      /* class  */ TIMER_EVENT,
...
void GObservable::fireGEvent(QWheelEvent *event, EventType eventType,
                             const std::string &eventName) {
  require::nonNull(event, "GObservable::fireGEvent", "event");
  GEvent wheelEvent(
      /* class  */ MOUSE_EVENT,
...
void GObservable::removeEventListeners(
    std::initializer_list<std::string> eventNames) {
  for (std::string eventName : eventNames) {
    removeEventListener(eventName);
  }
}

void GObservable::setEventListener(const std::string &eventName,
                                   GEventListener func) {
...
void GObservable::setEventListeners(
    std::initializer_list<std::string> eventNames, GEventListener func) {
  for (std::string eventName : eventNames) {
...
void GObservable::setEventListeners(
    std::initializer_list<std::string> eventNames, GEventListenerVoid func) {
  for (std::string eventName : eventNames) {
    setEventListener(eventName, func);
  }
}
...
GTimer::GTimer(double milliseconds) : _ms(0), _id(-1) {
  setDelay(milliseconds);
}

GTimer::~GTimer() { stop(); }

double GTimer::getDelay() const { return _ms; }

bool GTimer::isStarted() const { return _id >= 0; }
...
void GTimer::start() {
  _Internal_QMainWindow *lastWindow =
      static_cast<_Internal_QMainWindow *>(GWindow::getLastWindow());
  if (!lastWindow) {
    error("GTimer::start: You must create at least one GWindow before starting "
          "a GTimer.");
    return;
  }
  GThread::runOnQtGuiThreadAsync(
      [this, lastWindow]() { _id = lastWindow->timerStart(_ms); });
}
...
void GTimer::stop() {
  if (isStarted()) {
    _Internal_QMainWindow *lastWindow =
        static_cast<_Internal_QMainWindow *>(GWindow::getLastWindow());
    if (!lastWindow) {
      error("GTimer::constructor: You must create at least one GWindow before "
            "stopping a GTimer.");
      return;
    }
 ...
 * - destructor now stops event processing
 * @version 2018/11/27
 * - added code to set a widget visible after adding/inserting it to flow
 * container (needed to see widgets added to container after window is showing
 * on screen)
 * @version 2018/09/19
...
// margin  - around container, but outside of its background color area (like
// CSS) padding - around container, but within its background color area (like
// CSS) spacing - between neighboring widgets in container
...
GContainer::GContainer(Layout layout, QWidget *parent)
    : _iqcontainer(nullptr), _layout(layout) {
  GThread::runOnQtGuiThread([this, layout, parent]() {
    _iqcontainer =
        new _Internal_QContainer(this, layout, getInternalParent(parent));
  });
  setVisible(false); // all widgets are not shown until added to a window
}

GContainer::GContainer(Layout /*layout*/, int rows, int cols, QWidget *parent)
    : _iqcontainer(nullptr), _layout(LAYOUT_GRID) {
  GThread::runOnQtGuiThread([this, rows, cols, parent]() {
    _iqcontainer =
        new _Internal_QContainer(this, rows, cols, getInternalParent(parent));
  });
  setVisible(false); // all widgets are not shown until added to a window
}
...
void GContainer::add(GInteractor &interactor) { add(&interactor); }

void GContainer::addToGrid(GInteractor *interactor, int row, int col,
...
void GContainer::addToRegion(GInteractor *interactor, Region region) {
  ...
  GThread::runOnQtGuiThread(
      [this, widget, region]() { _iqcontainer->addToRegion(widget, region); });
}

void GContainer::addToRegion(GInteractor *interactor,
                             const std::string &region) {
  addToRegion(interactor, stringToRegion(region));
}
...
void GContainer::clear() {
  ...
  GThread::runOnQtGuiThread([this]() { _iqcontainer->clear(); });
}
...
void GContainer::clearRegion(Region region) {
  ...
  GThread::runOnQtGuiThread(
      [this, region]() { _iqcontainer->clearRegion(region); });
}
...
Vector<GInteractor *>
GContainer::getDescendents(const std::string &type) const {
  Vector<GInteractor *> result;
  for (GInteractor *interactor : _interactors) {
    // pre-order traversal; add parent and then visit children
    if (type.empty() || type == "*" ||
        equalsIgnoreCase(type, interactor->getType())) {
...
GInteractor *GContainer::getInteractor(int i) const { return _interactors[i]; }

const Vector<GInteractor *> &GContainer::getInteractors() const {
  return _interactors;
}

int GContainer::getInteractorCount() const { return _interactors.size(); }
...
GInteractor *
GContainer::getInteractorByRegion(int i, const std::string &region) const {
...
GContainer::Layout GContainer::getLayout() const { return _layout; }

double GContainer::getMargin() const { return _iqcontainer->getMargin(); }
...
double GContainer::getSpacing() const { return _iqcontainer->getSpacing(); }

std::string GContainer::getType() const { return "GContainer"; }
...
void GContainer::insert(int index, GInteractor *interactor) {
  ...
  GThread::runOnQtGuiThread(
      [this, index, widget]() { _iqcontainer->insert(index, widget); });
}
...
void GContainer::insertToRegion(int index, GInteractor *interactor,
                                Region region) {
...
bool GContainer::isEmpty() const { return getInteractorCount() == 0; }

bool GContainer::regionContains(GInteractor *interactor,
                                GContainer::Region region) const {
  if (!interactor) {
    return false;
  } else {
...
void GContainer::remove(GInteractor *interactor) {
  ...
  GThread::runOnQtGuiThread([this, widget]() { _iqcontainer->remove(widget); });
}

void GContainer::remove(GInteractor &interactor) { remove(&interactor); }
...
void GContainer::remove(int index) {
  ...
  GThread::runOnQtGuiThread([this, index]() { _iqcontainer->remove(index); });
}
...
void GContainer::removeFromRegion(GInteractor *interactor,
                                  const std::string &region) {
  removeFromRegion(interactor, stringToRegion(region));
}
...
void GContainer::setAlignment(HorizontalAlignment halign,
                              VerticalAlignment valign) {
...
void GContainer::setHorizontalAlignment(HorizontalAlignment halign) {
  GThread::runOnQtGuiThread(
      [this, halign]() { _iqcontainer->setHorizontalAlignment(halign); });
}

void GContainer::setMargin(double px) {
  GThread::runOnQtGuiThread([this, px]() { _iqcontainer->setMargin((int)px); });
}

void GContainer::setPadding(double px) { setPadding(px, px, px, px); }

void GContainer::setPadding(double topBottom, double leftRight) {
  setPadding(topBottom, leftRight, topBottom, leftRight);
}

void GContainer::setPadding(double top, double right, double bottom,
                            double left) {
...
void GContainer::setRegionAlignment(const std::string &region,
                                    const std::string &align) {
...
void GContainer::setRegionHorizontalAlignment(Region region,
                                              HorizontalAlignment halign) {
...
void GContainer::setRegionHorizontalAlignment(const std::string &region,
                                              const std::string &halign) {
  setRegionHorizontalAlignment(stringToRegion(region),
                               toHorizontalAlignment(halign));
}

void GContainer::setRegionVerticalAlignment(Region region,
                                            VerticalAlignment valign) {
...
void GContainer::setRegionVerticalAlignment(const std::string &region,
                                            const std::string &valign) {
  setRegionVerticalAlignment(stringToRegion(region),
                             toVerticalAlignment(valign));
}

void GContainer::setSpacing(double px) {
  GThread::runOnQtGuiThread(
      [this, px]() { _iqcontainer->setSpacing((int)px); });
}

void GContainer::setVerticalAlignment(VerticalAlignment valign) {
  GThread::runOnQtGuiThread(
      [this, valign]() { _iqcontainer->setVerticalAlignment(valign); });
}
...
_Internal_QContainer::_Internal_QContainer(GContainer *gcontainer,
                                           GContainer::Layout layoutType,
                                           QWidget *parent)
    : QWidget(parent), _gcontainer(gcontainer),
      _layoutType(GContainer::LAYOUT_NONE), _halign(ALIGN_CENTER),
      _valign(ALIGN_MIDDLE), _margin(GContainer::MARGIN_DEFAULT),
      _spacing(GContainer::SPACING_DEFAULT),
      // _rows(0),
      _cols(0), _currentIndex(-1), _overallLayout(nullptr),
      _northLayout(nullptr), _southLayout(nullptr), _westLayout(nullptr),
      _eastLayout(nullptr), _centerLayout(nullptr), _middleLayout(nullptr) {
...
_Internal_QContainer::_Internal_QContainer(GContainer *gcontainer, int /*rows*/,
    ...
    : QWidget(parent), _gcontainer(gcontainer),
      ...
      _spacing(GContainer::SPACING_DEFAULT),
      // _rows(rows),
      _cols(cols), _currentIndex(-1), _overallLayout(nullptr),
      _northLayout(nullptr), _southLayout(nullptr), _westLayout(nullptr),
      _eastLayout(nullptr), _centerLayout(nullptr), _middleLayout(nullptr) {
  setObjectName(QString::fromStdString("_Internal_QContainer_" +
                                       std::to_string(gcontainer->getID())));
...
void _Internal_QContainer::addToGrid(QWidget *widget, int row, int col,
                                     int rowspan, int colspan) {
...
void _Internal_QContainer::addToRegion(QWidget *widget,
  ...
  if (_layoutType == GContainer::LAYOUT_BORDER) {
    ...
    if (region == GContainer::REGION_CENTER) {
      ...
      layout->insertWidget(/* index */ layout->count() - 1, widget,
                           /* stretch */ 0);
    }
    widget->setVisible(true);
...
void _Internal_QContainer::fixAlignment(QWidget *widget,
                                        GContainer::Region region) {
  ...
  if (_layoutType == GContainer::LAYOUT_BORDER) {
    ...
    if (_halignMap.containsKey(region) && _valignMap.containsKey(region)) {
      layout->setAlignment(widget, toQtAlignment(_halignMap[region]) |
                                       toQtAlignment(_valignMap[region]));
    ...
    layout()->setAlignment(widget,
                           toQtAlignment(_halign) | toQtAlignment(_valign));
  }
}
...
int _Internal_QContainer::getMargin() const { return _margin; }
...
QLayout *_Internal_QContainer::getQLayout() const { return layout(); }

int _Internal_QContainer::getSpacing() const { return _spacing; }
...
void _Internal_QContainer::insertToRegion(int i, QWidget *widget,
                                          GContainer::Region region) {
  if (_layoutType == GContainer::LAYOUT_BORDER) {
    QLayout *layout = layoutForRegion(region);
    if (region == GContainer::REGION_CENTER) {
...
QLayout *
_Internal_QContainer::layoutForRegion(GContainer::Region region) const {
...
bool _Internal_QContainer::regionContains(QWidget *widget,
                                          GContainer::Region region) const {
  if (_layoutType == GContainer::LAYOUT_BORDER) {
    QLayout *layout = layoutForRegion(region);
...
void _Internal_QContainer::removeFromRegion(QWidget *widget,
                                            GContainer::Region region) {
  if (_layoutType == GContainer::LAYOUT_BORDER) {
    QLayout *layout = layoutForRegion(region);
...
void _Internal_QContainer::setPadding(int top, int right, int bottom,
                                      int left) {
...
void _Internal_QContainer::setLayoutType(GContainer::Layout layoutType) {
  ...
  case GContainer::LAYOUT_BORDER: {
    ...
    _overallLayout->setObjectName(QString::fromStdString(
        "_overallLayout_" + std::to_string(_gcontainer->getID())));
    _northLayout = new QHBoxLayout;
    _northLayout->setObjectName(QString::fromStdString(
        "_northLayout_" + std::to_string(_gcontainer->getID())));
    _southLayout = new QHBoxLayout;
    _southLayout->setObjectName(QString::fromStdString(
        "_southLayout_" + std::to_string(_gcontainer->getID())));
    _westLayout = new QVBoxLayout;
    _westLayout->setObjectName(QString::fromStdString(
        "_westLayout_" + std::to_string(_gcontainer->getID())));
    _eastLayout = new QVBoxLayout;
    _eastLayout->setObjectName(QString::fromStdString(
        "_eastLayout_" + std::to_string(_gcontainer->getID())));
    _centerLayout = new QHBoxLayout;
    _centerLayout->setObjectName(QString::fromStdString(
        "_centerLayout_" + std::to_string(_gcontainer->getID())));
    _middleLayout = new QHBoxLayout;
    _middleLayout->setObjectName(QString::fromStdString(
        "_middleLayout_" + std::to_string(_gcontainer->getID())));
...
void _Internal_QContainer::setRegionHorizontalAlignment(
    GContainer::Region region, HorizontalAlignment halign) {
  ...
  if (layout == _westLayout || layout == _eastLayout ||
    ...
    // set each widget's horizontal alignment individually
    Qt::Alignment qtAlign = toQtAlignment(halign);
    QSizePolicy::Policy hSizePolicy = ((qtAlign & Qt::AlignJustify) != 0)
                                          ? QSizePolicy::Expanding
    ...
    int beforeStretch =
        (halign == ALIGN_CENTER || halign == ALIGN_RIGHT) ? 99 : 0;
    int afterStretch =
        (halign == ALIGN_LEFT || halign == ALIGN_CENTER) ? 99 : 0;
...
void _Internal_QContainer::setRegionStretch(GContainer::Region region,
                                            bool stretch) {
...
void _Internal_QContainer::setRegionVerticalAlignment(
    GContainer::Region region, VerticalAlignment valign) {
  ...
  if (layout == _westLayout || layout == _eastLayout) {
    ...
    boxLayout->removeItem(layout->itemAt(layout->count() - 1));
    int beforeStretch =
        (valign == ALIGN_MIDDLE || valign == ALIGN_BOTTOM) ? 99 : 0;
    ...
    QSizePolicy::Policy vSizePolicy = ((qtAlign & Qt::AlignJustify) != 0)
                                          ? QSizePolicy::Expanding
                                          : QSizePolicy::Preferred;
    for (int i = 0; i < layout->count(); i++) {
      QWidget *widget = layout->itemAt(i)->widget();
...
GScrollBar::GScrollBar(GScrollBar::Orientation orientation, int value,
                       int extent, int min, int max, QWidget *parent) {
  GThread::runOnQtGuiThread([this, orientation, parent]() {
    _iqscrollbar = new _Internal_QScrollBar(
        this, orientation == VERTICAL ? Qt::Vertical : Qt::Horizontal,
        getInternalParent(parent));
  });
...
std::string GScrollBar::getActionEventType() const { return "change"; }

int GScrollBar::getExtent() const { return _iqscrollbar->pageStep(); }

_Internal_QWidget *GScrollBar::getInternalWidget() const {
  return _iqscrollbar;
}

int GScrollBar::getMax() const { return _iqscrollbar->maximum(); }

int GScrollBar::getMin() const { return _iqscrollbar->minimum(); }

GScrollBar::Orientation GScrollBar::getOrientation() const {
  return _iqscrollbar->orientation() == Qt::Vertical ? VERTICAL : HORIZONTAL;
}

std::string GScrollBar::getType() const { return "QGScrollBar"; }

int GScrollBar::getValue() const { return _iqscrollbar->value(); }
...
void GScrollBar::setExtent(int extent) {
  GThread::runOnQtGuiThread(
      [this, extent]() { _iqscrollbar->setPageStep(extent); });
}
...
void GScrollBar::setMax(int max) {
  ...
  require::require(min <= max, "GScrollBar::setMax",
                   "max (" + std::to_string(max) +
                       ...
                       ")");
  GThread::runOnQtGuiThread([this, max]() { _iqscrollbar->setMaximum(max); });
  updateSize();
}
...
void GScrollBar::setMin(int min) {
  ...
  require::require(min <= max, "GScrollBar::setMin",
                   "min (" + std::to_string(min) +
                       ") cannot be greater than max (" + std::to_string(max) +
                       ")");
  GThread::runOnQtGuiThread([this, min]() { _iqscrollbar->setMinimum(min); });
  updateSize();
}
...
void GScrollBar::setValue(int value) {
  ...
  GThread::runOnQtGuiThread([this, value]() { _iqscrollbar->setValue(value); });
}
...
void GScrollBar::updateSize() {
  GThread::runOnQtGuiThread([this]() {
    ...
    if (getOrientation() == VERTICAL) {
      _iqscrollbar->setPreferredSize(_iqscrollbar->sizeHint().width(),
                                     getMax() - getMin() + 1);
    } else {
      _iqscrollbar->setPreferredSize(getMax() - getMin() + 1,
                                     _iqscrollbar->sizeHint().height());
    }
  });
}

_Internal_QScrollBar::_Internal_QScrollBar(GScrollBar *gscrollbar,
                                           Qt::Orientation orientation,
                                           QWidget *parent)
    : QScrollBar(orientation, parent), _gscrollbar(gscrollbar) {
  require::nonNull(gscrollbar, "_Internal_QScrollBar::constructor");
  setObjectName(QString::fromStdString("_Internal_QScrollBar_" +
                                       std::to_string(gscrollbar->getID())));
  connect(this, SIGNAL(valueChanged(int)), this, SLOT(handleValueChange(int)));
}

void _Internal_QScrollBar::detach() { _gscrollbar = nullptr; }
...
GLabel::GLabel(const std::string &text, const std::string &iconFileName,
               QWidget *parent)
    : _gtext(nullptr) {
  GThread::runOnQtGuiThread([this, parent]() {
...
GText *GLabel::getGText() const { return _gtext; }

_Internal_QWidget *GLabel::getInternalWidget() const { return _iqlabel; }

std::string GLabel::getLabel() const { return getText(); }

std::string GLabel::getText() const { return _iqlabel->text().toStdString(); }
...
std::string GLabel::getType() const { return "GLabel"; }

QWidget *GLabel::getWidget() const { return static_cast<QWidget *>(_iqlabel); }

bool GLabel::hasGText() const { return _gtext != nullptr; }

bool GLabel::isWordWrap() const { return _iqlabel->wordWrap(); }
...
void GLabel::setIcon(const QIcon &icon) {
  ...
  GThread::runOnQtGuiThread([this, &icon]() {
    ...
    if (!getText().empty()) {
      std::cerr << "Warning: a GLabel cannot currently have both text and icon."
                << std::endl;
    }
  });
}
  ...
  if (!filename.empty() && fileExists(filename)) {
    GThread::runOnQtGuiThread([this, filename, retainIconSize]() {
      if (filename.empty()) {
        ...
        if (!getText().empty()) {
          std::cerr
              << "Warning: a GLabel cannot currently have both text and icon."
              << std::endl;
        }
      }
    });
...
void GLabel::setLabel(const std::string &text) { setText(text); }
...
void GLabel::setTextPosition(GInteractor::TextPosition position) {
  // TODO: doesn't really work because a label can't have both text and icon in
  // Qt
...
void GLabel::setWordWrap(bool wrap) {
  GThread::runOnQtGuiThread([this, wrap]() { _iqlabel->setWordWrap(wrap); });
}
...
_Internal_QLabel::_Internal_QLabel(GLabel *glabel, QWidget *parent)
    : QLabel(parent), _glabel(glabel) {
  require::nonNull(glabel, "_Internal_QLabel::constructor");
  setObjectName(QString::fromStdString("_Internal_QLabel_" +
                                       std::to_string(glabel->getID())));
}

void _Internal_QLabel::detach() { _glabel = nullptr; }
 ...
 * @author Marty Stepp
 * @version 2019/08/13
 * - bug fix for loading GImage (hasError -> !hasError) - thanks to Tyler
 * Conklin
 * @version 2019/05/05
...
GObject::GObject(double x, double y, double width, double height)
    : _x(x), _y(y), _width(width), _height(height), _lineWidth(1),
      _opacity(1.0), _lineStyle(GObject::LINE_SOLID), _color(""), _colorInt(0),
      _fillColor(""), _fillColorInt(0), _font(""), _fillFlag(false),
      _visible(true), _transformed(false), _parent(nullptr) {
...
double GObject::getBottomY() const { return getY() + getHeight(); }
...
double GObject::getCenterX() const { return getX() + getWidth() / 2; }

double GObject::getCenterY() const { return getY() + getHeight() / 2; }

std::string GObject::getColor() const { return _color; }

std::string GObject::getFillColor() const { return _fillColor; }

double GObject::getHeight() const { return _height; }

GObject::LineStyle GObject::getLineStyle() const { return _lineStyle; }

double GObject::getLineWidth() const { return _lineWidth; }

GPoint GObject::getLocation() const { return GPoint(getX(), getY()); }

double GObject::getOpacity() const { return _opacity; }

GCompound *GObject::getParent() const { return _parent; }

double GObject::getRightX() const { return getX() + getWidth(); }
...
double GObject::getWidth() const { return _width; }

double GObject::getX() const { return _x; }

double GObject::getY() const { return _y; }
...
bool GObject::isAntiAliasing() { return _sAntiAliasing; }

bool GObject::isFilled() const { return _fillFlag; }

bool GObject::isTransformed() const { return _transformed; }

bool GObject::isVisible() const { return _visible; }
...
void GObject::setAntiAliasing(bool value) { _sAntiAliasing = value; }
...
void GObject::setFont(const QFont &font) { setFont(GFont::toFontString(font)); }
...
void GObject::setForeground(int r, int g, int b) { setColor(r, g, b); }

void GObject::setForeground(int rgb) { setColor(rgb); }

void GObject::setForeground(const std::string &color) { setColor(color); }

void GObject::setHeight(double height) { setSize(getWidth(), height); }
...
void GObject::setWidth(double width) { setSize(width, getHeight()); }
...
std::string GObject::toString() const {
  ...
  return getType() + "(" + "x=" + std::to_string(_x) +
         ",y=" + std::to_string(_y) + ",w=" + std::to_string(_width) +
...
std::string GObject::toStringExtra() const { return ""; }

GArc::GArc(double width, double height, double start, double sweep)
    : GObject(/* x */ 0, /* y */ 0, width, height), _start(start),
      _sweep(sweep) {
  // empty
}

GArc::GArc(double x, double y, double width, double height, double start,
           double sweep)
    : GObject(x, y, width, height), _start(start), _sweep(sweep) {
  // empty
}
...
void GArc::draw(QPainter *painter) {
  ...
  painter->drawChord((int)getX(), (int)getY(), (int)getWidth(),
                     (int)getHeight(), (int)(_start * QT_ANGLE_SCALE_FACTOR),
...
GPoint GArc::getEndPoint() const { return getArcPoint(_start + _sweep); }

GRectangle GArc::getFrameRectangle() const { return getBounds(); }

double GArc::getStartAngle() const { return _start; }

GPoint GArc::getStartPoint() const { return getArcPoint(_start); }

double GArc::getSweepAngle() const { return _sweep; }

std::string GArc::getType() const { return "GArc"; }
...
GCompound::GCompound() : _autoRepaint(true) {
  // empty
}
...
void GCompound::add(GObject &gobj) { add(&gobj); }

void GCompound::add(GObject &gobj, double x, double y) { add(&gobj, x, y); }
...
GObject *GCompound::getElement(int index) const { return _contents.get(index); }
...
int GCompound::getElementCount() const { return _contents.size(); }

std::string GCompound::getType() const { return "GCompound"; }

QWidget *GCompound::getWidget() const { return _widget; }

bool GCompound::isAutoRepaint() const { return _autoRepaint; }

bool GCompound::isEmpty() const { return _contents.size() == 0; }
...
void GCompound::remove(GObject &gobj) { remove(&gobj); }
...
void GCompound::repaintRegion(const GRectangle &bounds) {
  repaintRegion((int)bounds.getX(), (int)bounds.getY(), (int)bounds.getWidth(),
...
void GCompound::setAutoRepaint(bool autoRepaint) { _autoRepaint = autoRepaint; }

void GCompound::setWidget(QWidget *widget) { _widget = widget; }

std::string GCompound::toString() const { return "GCompound(...)"; }

GImage::GImage(const std::string &filename, double x, double y)
    : GObject(x, y), _filename(filename), _qimage(nullptr) {
  if (!load(filename)) {
...
GImage::GImage(std::istream &source, double x, double y)
    : GObject(x, y), _filename("std::istream source"), _qimage(nullptr) {
  if (!loadFromStream(source)) {
...
GImage::GImage(double width, double height) {
  require::nonNegative2D(width, height, "GImage::constructor", "width",
                         "height");
  _width = width;
  _height = height;
  GThread::runOnQtGuiThread([this]() {
    _qimage = new QImage(static_cast<int>(_width), static_cast<int>(_height),
                         QImage::Format_ARGB32);
  });
}
...
bool GImage::loadFromStream(std::istream &input) {
  ...
  GThread::runOnQtGuiThread([&, this]() {
    _qimage = new QImage;
    if (_qimage->loadFromData(reinterpret_cast<const uchar *>(bytes.data()),
                              bytes.size())) {
      _width = _qimage->width();
      _height = _qimage->height();
    } else {
...
void GImage::draw(QPainter *painter) {
  ...
  painter->setOpacity(_opacity);
  painter->setTransform(_transform, /* combine */ false);
  if (floatingPointEqual(myWidth, imgWidth) &&
      floatingPointEqual(myHeight, imgHeight)) {
    // draw unscaled
...
std::string GImage::getFileName() const { return _filename; }

int GImage::getPixel(int x, int y) const {
  require::inRange2D(x, y, (int)getWidth() - 1, (int)getHeight() - 1,
                     "GImage::getPixel", "x", "y");
  return (int)_qimage->pixel(x, y);
}

QImage *GImage::getQImage() const { return _qimage; }

std::string GImage::getType() const { return "GImage"; }

void GImage::setPixel(int x, int y, int rgb) { _qimage->setPixel(x, y, rgb); }
...
GLine::GLine(double x0, double y0, double x1, double y1,
             GObject::LineStyle lineStyle)
    : GObject(x0, y0), _dx(x1 - x0), _dy(y1 - y0) {
  setPoints(x0, y0, x1, y1); // checks if point swap is needed
  setLineStyle(lineStyle);
}

GLine::GLine(const GPoint &p0, const GPoint &p1)
    : GObject(p0.getX(), p0.getY()), _dx(p1.getX() - p0.getX()),
      _dy(p1.getY() - p0.getY()) {
  // empty
}
...
bool GLine::contains(double x, double y) const {
  ...
  double y1 = getEndY();
  double tSquared =
      STATIC_VARIABLE(LINE_TOLERANCE) * STATIC_VARIABLE(LINE_TOLERANCE);
  ...
  double u =
      ((x - x0) * (x1 - x0) + (y - y0) * (y1 - y0)) / dsq(x0, y0, x1, y1);
...
void GLine::draw(QPainter *painter) {
  ...
  painter->drawLine(static_cast<int>(getStartX()),
                    static_cast<int>(getStartY()), static_cast<int>(getEndX()),
...
GPoint GLine::getEndPoint() const { return GPoint(getX() + _dx, getY() + _dy); }

double GLine::getEndX() const { return getX() + _dx; }

double GLine::getEndY() const { return getY() + _dy; }

double GLine::getHeight() const { return std::fabs(_dy); }

GPoint GLine::getStartPoint() const { return getLocation(); }

double GLine::getStartX() const { return getX(); }

double GLine::getStartY() const { return getY(); }

std::string GLine::getType() const { return "GLine"; }

double GLine::getWidth() const { return std::fabs(_dx); }

void GLine::setEndPoint(double x1, double y1) {
  setPoints(getStartX(), getStartY(), x1, y1);
}

void GLine::setEndPoint(const GPoint &p) { setEndPoint(p.getX(), p.getY()); }
...
void GOval::draw(QPainter *painter) {
  ...
  initializeBrushAndPen(painter);
  painter->drawEllipse((int)getX(), (int)getY(), (int)getWidth(),
                       (int)getHeight());
}

std::string GOval::getType() const { return "GOval"; }
...
void GPolygon::addEdge(double dx, double dy) { addVertex(_cx + dx, _cy + dy); }

void GPolygon::addEdge(const GPoint &pt) { addEdge(pt.getX(), pt.getY()); }
...
void GPolygon::addVertex(const GPoint &pt) { addVertex(pt.getX(), pt.getY()); }
...
double GPolygon::getHeight() const { return getBounds().getHeight(); }

std::string GPolygon::getType() const { return "GPolygon"; }

GPoint GPolygon::getVertex(int i) const {
  return GPoint(_vertices[i].x(), _vertices[i].y());
}

int GPolygon::getVertexCount() const { return _vertices.size(); }
...
double GPolygon::getWidth() const { return getBounds().getHeight(); }
...
void GRect::draw(QPainter *painter) {
  ...
  painter->drawRect((int)getX(), (int)getY(), (int)getWidth(),
                    (int)getHeight());
  painter->setRenderHint(QPainter::Antialiasing, GObject::isAntiAliasing());
}

std::string GRect::getType() const { return "GRect"; }
...
GRoundRect::GRoundRect(double width, double height, double corner)
    : GRect(/* x */ 0, /* y */ 0, width, height), _corner(corner) {
...
GRoundRect::GRoundRect(double x, double y, double width, double height,
                       double corner)
    : GRect(x, y, width, height), _corner(corner) {
  require::nonNegative(corner, "GRoundRect::constructor", "corner");
}
...
bool GRoundRect::contains(double x, double y) const {
  ...
  // JL BUGFIX: The rest of this is code to return correct result in
  // non-transformed case (accounting for corners)
  if (!getBounds().contains(x, y)) {
    return false;
  }

  // If corner diameter is too big, the largest sensible value is used by Java
  // back end.
...
void GRoundRect::draw(QPainter *painter) {
  ...
  initializeBrushAndPen(painter);
  painter->drawRoundRect((int)getX(), (int)getY(), (int)getWidth(),
                         (int)getHeight(), (int)_corner, (int)_corner);
}

double GRoundRect::getCorner() const { return _corner; }

std::string GRoundRect::getType() const { return "GRoundRect"; }
...
std::string GText::getFont() const { return _font; }
...
std::string GText::getLabel() const { return _text; }

std::string GText::getText() const { return _text; }

std::string GText::getType() const { return "GText"; }

void GText::setFont(const QFont &font) { setFont(GFont::toFontString(font)); }
...
void GText::setText(const std::string &str) { setLabel(str); }

std::string GText::toStringExtra() const { return "text=\"" + _text + "\""; }
...
GButton::GButton(const std::string &text, const std::string &iconFileName,
                 QWidget *parent) {
  GThread::runOnQtGuiThread([this, parent]() {
...
GButton::GButton(const std::string &text, const QPixmap &icon,
                 QWidget *parent) {
  GThread::runOnQtGuiThread([this, parent]() {
    _iqpushbutton = new _Internal_QPushButton(this, getInternalParent(parent));
  });
...
_Internal_QWidget *GButton::getInternalWidget() const { return _iqpushbutton; }
...
std::string GButton::getType() const { return "GButton"; }
...
void GButton::setAccelerator(const std::string &accelerator) {
  GThread::runOnQtGuiThread([this, accelerator]() {
    QKeySequence keySeq(
        QString::fromStdString(normalizeAccelerator(accelerator)));
...
void GButton::setText(const std::string &text) {
  GThread::runOnQtGuiThread(
      [this, text]() { _iqpushbutton->setText(QString::fromStdString(text)); });
...
void GButton::setTextPosition(SwingConstants /*horizontal*/,
...
_Internal_QPushButton::_Internal_QPushButton(GButton *button, QWidget *parent)
    : QToolButton(parent), _gbutton(button) {
  require::nonNull(button, "_Internal_QPushButton::constructor");
  setObjectName(QString::fromStdString("_Internal_QPushButton_" +
                                       std::to_string(button->getID())));
  setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
  connect(this, SIGNAL(clicked()), this, SLOT(handleClick()));
}

void _Internal_QPushButton::detach() { _gbutton = nullptr; }
...
void _Internal_QPushButton::mouseDoubleClickEvent(QMouseEvent *event) {
  require::nonNull(event, "_Internal_QPushButton::mouseDoubleClickEvent",
                   "event");
  QWidget::mouseDoubleClickEvent(event); // call super
...
std::string GFontChooser::showDialog(const std::string &title,
                                     const std::string &initialFont) {
  return showDialog(/* parent */ (QWidget *)nullptr, title, initialFont);
}

std::string GFontChooser::showDialog(GWindow *parent, const std::string &title,
                                     const std::string &initialFont) {
  return showDialog(parent ? parent->getWidget() : nullptr, title, initialFont);
}

std::string GFontChooser::showDialog(QWidget *parent, const std::string &title,
                                     const std::string &initialFont) {
  QFont initialQFont = GFont::toQFont(initialFont);
  std::string result = "";
  GThread::runOnQtGuiThread([parent, title, initialQFont, &result]() {
    bool ok = false;
    QFont font = QFontDialog::getFont(&ok, initialQFont, parent,
...
std::string GFileChooser::showOpenDialog(const std::string &title,
                                         const std::string &currentDir,
                                         const std::string &fileFilter) {
  return showOpenDialog(/* parent */ (QWidget *)nullptr, title, currentDir,
                        fileFilter);
}

std::string GFileChooser::showOpenDialog(GWindow *parent,
                                         const std::string &title,
                                         const std::string &currentDir,
                                         const std::string &fileFilter) {
  return showOpenDialog(parent ? parent->getWidget() : nullptr, title,
                        currentDir, fileFilter);
}

std::string GFileChooser::showOpenDialog(QWidget *parent,
                                         const std::string &title,
                                         const std::string &currentDir,
                                         const std::string &fileFilter) {
  // Qt filter spec:
  ...
  GThread::runOnQtGuiThread([parent, title, currentDir, fileFilter, &result]() {
    result = QFileDialog::getOpenFileName(
                 ...
                 QString::fromStdString(normalizeFileFilter(fileFilter)))
                 .toStdString();
  });
  return result;
}

std::string GFileChooser::showSaveDialog(const std::string &title,
                                         const std::string &currentDir,
                                         const std::string &fileFilter) {
  return showSaveDialog(/* parent */ (QWidget *)nullptr, title, currentDir,
                        fileFilter);
}

std::string GFileChooser::showSaveDialog(GWindow *parent,
                                         const std::string &title,
                                         const std::string &currentDir,
                                         const std::string &fileFilter) {
  return showSaveDialog(parent ? parent->getWidget() : nullptr, title,
                        currentDir, fileFilter);
}

std::string GFileChooser::showSaveDialog(QWidget *parent,
                                         const std::string &title,
                                         const std::string &currentDir,
                                         const std::string &fileFilter) {
  std::string result = "";
 ...
 * @author Marty Stepp
 * @version 2019/04/20
 * - display expected/actual values using toPrintable to show non-printable
 * characters better
 * @version 2018/10/06
...
void GDiffGui::showDialog(const std::string &name1, const std::string &text1,
                          const std::string &name2, const std::string &text2,
                          int diffFlags, bool showCheckBoxes) {
  /* GDiffGui* diffGui = */ new GDiffGui(name1, text1, name2, text2, diffFlags,
                                         showCheckBoxes);
  // TODO: delete diffGui;
...
GDiffGui::GDiffGui(const std::string &name1, const std::string &text1,
                   const std::string &name2, const std::string &text2,
                   int diffFlags, bool /*showCheckBoxes*/) {
  GThread::runOnQtGuiThread([this, name1, text1, name2, text2, diffFlags]() {
    ...
    _hsplitter = new QSplitter(/* orientation */ Qt::Horizontal,
                               /* parent */ _window->getWidget());
    ...
    auto windowCloseLambda = [this](GEvent event) {
      if (event.getType() == KEY_PRESSED &&
          event.getKeyChar() == GEvent::ESCAPE_KEY) {
        _window->close();
      }
    };
    ...
    _textAreaLeft->setEventListener(
        "scroll", [this]() { syncScrollBars(/* left */ true); });
    ...
    _textAreaRight->setEventListener(
        "scroll", [this]() { syncScrollBars(/* left */ false); });
    ...
    _hsplitter->setSizes(
        QList<int>({INT_MAX, INT_MAX})); // evenly size the two halves
    _hsplitterInteractor = new GGenericInteractor<QSplitter>(_hsplitter);

    _vsplitter = new QSplitter(/* orientation */ Qt::Vertical,
                               /* parent */ _window->getWidget());
...
void GDiffGui::setupDiffText(const std::string &diffs) {
  ...
  for (std::string line : lines) {
    ...
    if (startsWith(line, "EXPECTED <")) {
      color = GWindow::chooseLightDarkModeColor(COLOR_EXPECTED,
                                                COLOR_EXPECTED_DARK_MODE);
    } else if (startsWith(line, "STUDENT  >")) {
      color = GWindow::chooseLightDarkModeColor(COLOR_STUDENT,
                                                COLOR_STUDENT_DARK_MODE);
    } else {
      color = COLOR_NORMAL;
    }
...
void GDiffGui::setupLeftRightText(GTextArea *textArea,
  ...
  for (int i = 0; i < lines.size(); i++) {
    ...
    textArea->appendFormattedText(
        lineNumberString,
        GWindow::chooseLightDarkModeColor(COLOR_LINE_NUMBERS,
...
GOptionPane::ConfirmResult
GOptionPane::showConfirmDialog(const std::string &message,
                               const std::string &title, ConfirmType type) {
  return showConfirmDialog(/* parent */ static_cast<QWidget *>(nullptr),
                           message, title, type);
}

GOptionPane::ConfirmResult
GOptionPane::showConfirmDialog(GWindow *parent, const std::string &message,
                               const std::string &title, ConfirmType type) {
  return showConfirmDialog(parent ? parent->getWidget() : nullptr, message,
                           title, type);
}

GOptionPane::ConfirmResult
GOptionPane::showConfirmDialog(QWidget *parent, const std::string &message,
                               const std::string &title, ConfirmType type) {
  ...
  std::string titleToUse =
      title.empty() ? std::string("Select an option") : title;
  ...
  GThread::runOnQtGuiThread(
      [parent, titleToUse, message, buttons, defaultButton, &confirmResult]() {
        int dialogResult = QMessageBox::question(
            parent, QString::fromStdString(titleToUse),
            QString::fromStdString(message), buttons, defaultButton);
        switch (dialogResult) {
...
std::string GOptionPane::showInputDialog(const std::string &message,
  ...
  return showInputDialog(/* parent */ static_cast<QWidget *>(nullptr), message,
                         title, initialValue);
}
...
std::string GOptionPane::showInputDialog(GWindow *parent,
                                         ...
                                         const std::string &title,
                                         const std::string &initialValue) {
  return showInputDialog(parent ? parent->getWidget() : nullptr, message, title,
                         initialValue);
}
...
std::string GOptionPane::showInputDialog(QWidget *parent,
  ...
  std::string result = "";
  GThread::runOnQtGuiThread([parent, titleToUse, message, initialValue,
                             &result]() {
    result = QInputDialog::getText(parent, QString::fromStdString(titleToUse),
                                   QString::fromStdString(message),
                                   QLineEdit::Normal,
                                   QString::fromStdString(initialValue))
                 .toStdString();
  });
  return result;
}
...
void GOptionPane::showMessageDialog(const std::string &message,
                                    const std::string &title,
                                    MessageType type) {
  showMessageDialog(/* parent */ static_cast<QWidget *>(nullptr), message,
                    title, type);
}

void GOptionPane::showMessageDialog(GWindow *parent, const std::string &message,
                                    const std::string &title,
                                    MessageType type) {
  showMessageDialog(parent ? parent->getWidget() : nullptr, message, title,
                    type);
}

void GOptionPane::showMessageDialog(QWidget *parent, const std::string &message,
                                    const std::string &title,
                                    MessageType type) {
  ...
  GThread::runOnQtGuiThread([parent, message, titleToUse, type]() {
    if (type == GOptionPane::MessageType::MESSAGE_PLAIN ||
        type == GOptionPane::MessageType::MESSAGE_INFORMATION ||
        type == GOptionPane::MessageType::MESSAGE_QUESTION) {
      QMessageBox::information(parent, QString::fromStdString(titleToUse),
                               QString::fromStdString(message));
    } else if (type == GOptionPane::MessageType::MESSAGE_WARNING) {
      QMessageBox::warning(parent, QString::fromStdString(titleToUse),
                           QString::fromStdString(message));
    } else if (type == GOptionPane::MessageType::MESSAGE_ERROR) {
      QMessageBox::critical(parent, QString::fromStdString(titleToUse),
                            QString::fromStdString(message));
    } else if (type == GOptionPane::MessageType::MESSAGE_ABOUT) {
      QMessageBox::about(parent, QString::fromStdString(titleToUse),
                         QString::fromStdString(message));
    }
  });
}

std::string GOptionPane::showOptionDialog(
    const std::string &message, const Vector<std::string> &options,
    const std::string &title, const std::string &initiallySelected) {
  return showOptionDialog(/* parent */ static_cast<QWidget *>(nullptr), message,
                          options, title, initiallySelected);
}

std::string
GOptionPane::showOptionDialog(GWindow *parent, const std::string &message,
                              const Vector<std::string> &options,
                              const std::string &title,
                              const std::string &initiallySelected) {
  return showOptionDialog(parent ? parent->getWidget() : nullptr, message,
                          options, title, initiallySelected);
}

std::string
GOptionPane::showOptionDialog(QWidget *parent, const std::string &message,
                              const Vector<std::string> &options,
                              const std::string &title,
                              const std::string &initiallySelected) {
  std::string titleToUse =
      title.empty() ? std::string("Select an option") : title;
  std::string result = "";
  GThread::runOnQtGuiThread([parent, message, &options, titleToUse,
                             initiallySelected, &result]() {
    QMessageBox box;
    if (parent) {
    ...
    for (int i = 0; i <= 2; i++) {
      ...
      for (QAbstractButton *button : box.buttons()) {
        ...
        button->setText(
            QString::fromStdString(text.substr(0, i) + "&" + text.substr(i)));
        button->setShortcut(
            QKeySequence::fromString(QString::fromStdString(letter)));
        buttonIndex++;
      }
    }
...
void GOptionPane::showTextFileDialog(const std::string &fileText,
                                     const std::string &title, int rows,
                                     int cols) {
  showTextFileDialog(static_cast<QWidget *>(nullptr), fileText, title, rows,
                     cols);
}
...
void GOptionPane::showTextFileDialog(GWindow *parent,
                                     ...
                                     const std::string &title, int rows,
                                     int cols) {
  showTextFileDialog(parent ? parent->getWidget() : nullptr, fileText, title,
                     rows, cols);
}
...
void GOptionPane::showTextFileDialog(QWidget * /*parent*/,
  ...
  std::string titleToUse =
      title.empty() ? std::string("Text file contents") : title;
  ...
  button->setActionListener([window]() { window->close(); });
...
QtGui::QtGui() : _initialized(false) {
  connect(GEventQueue::instance(), SIGNAL(eventReady()), this,
          SLOT(processEventFromQueue()));
}

void QtGui::exitGraphics(int exitCode) {
  if (_app) {
// need to temporarily turn off C++ lib exit macro to call QApplication's exit
// method (NOTE: must keep in sync with exit definition in init.h)
#undef exit
    _app->quit();
    _app = nullptr;
    std::exit(exitCode);
...
QSPLApplication *QtGui::getApplication() { return _app; }

int QtGui::getArgc() const { return _argc; }

char **QtGui::getArgv() const { return _argv; }
...
void QtGui::initializeQt() {
  ...
  GThread::runOnQtGuiThread([this]() {
    if (!_app) {
      qSetMessagePattern("Qt internal warning: %{message}\n"
                         "  - pid: %{pid}\n"
                         "  - thread: %{threadid}\n"
...
void endOfLibraryStudentThread() {
  ...
  if (getConsoleEnabled()) {
    ...
    // BUGFIX: no, this is not needed and is bad; it exits the window too soon;
    // disable QtGui::instance()->exitGraphics(result);
  }
}
} // namespace stanfordcpplib
...
bool getConsoleClearEnabled() { return true; }

/* GWindow::CloseOperation */ int getConsoleCloseOperation() { return 0; }

bool getConsoleEcho() { return true; }

bool getConsoleEnabled() { return true; }

bool getConsoleEventOnClose() { return true; }

bool getConsoleExitProgramOnClose() { return true; }

std::string getConsoleFont() { return ""; }

double getConsoleHeight() { return 0; }

bool getConsoleLocationSaved() { return false; }

bool getConsolePrintExceptions() {
  return exceptions::getTopLevelExceptionHandlerEnabled();
}

bool getConsoleSettingsLocked() { return false; }

double getConsoleWidth() { return 0; }

std::string getConsoleWindowTitle() { return ""; }
...
void clearConsole() { GConsoleWindow::instance()->clearConsole(); }
...
bool getConsoleEcho() { return GConsoleWindow::instance()->isEcho(); }
...
double getConsoleHeight() { return GConsoleWindow::instance()->getHeight(); }
...
GDimension getConsoleSize() { return GConsoleWindow::instance()->getSize(); }

double getConsoleWidth() { return GConsoleWindow::instance()->getWidth(); }

GConsoleWindow *getConsoleWindow() { return GConsoleWindow::instance(); }
...
void setConsolePropertiesQt() {
  ...
  std::string fontStr =
      std::string("Monospaced-Bold-") + std::to_string(SPL_CONSOLE_FONTSIZE);
...
GDrawingSurface::GDrawingSurface()
    : _forwardTarget(nullptr), _backgroundColor("0xffffffff"), _color(""),
      _fillColor(""), _font(""), _backgroundColorInt(0xffffffff), _colorInt(0),
      _fillColorInt(0), _lineStyle(GObject::LINE_SOLID), _lineWidth(1),
      _autoRepaint(true) {
  // empty
}

GDrawingSurface::~GDrawingSurface() { _forwardTarget = nullptr; }

void GDrawingSurface::checkBounds(const std::string &member, double x, double y,
                                  double width, double height) const {
  require::inRange2D(x, y, width - 1, height - 1, member);
}

void GDrawingSurface::checkColor(const std::string & /* member */,
                                 int /* rgb */) const {
  ...
  //        error(member + ": color is outside of range 0x000000 through
  //        0xffffff");
  //    }
}

void GDrawingSurface::checkSize(const std::string & /* member */,
                                double /* width */, double /* height */) const {
  //    if (width < 0 || height < 0) {
  ...
  //        error(getType() + "::" + member + ": width/height too large (cannot
  //        exceed "
...
void GDrawingSurface::conditionalRepaintRegion(int x, int y, int width,
                                               int height) {
  if (isAutoRepaint()) {
    repaintRegion(x, y, width, height);
  }
...
void GDrawingSurface::draw(GObject &gobj) { draw(&gobj); }
...
void GDrawingSurface::drawArc(double x, double y, double width, double height,
...
void GDrawingSurface::drawImage(const std::string &filename, double x,
                                double y) {
...
void GDrawingSurface::drawOval(double x, double y, double width,
                               double height) {
...
GPoint GDrawingSurface::drawPolarLine(const GPoint &p0, double r,
                                      double theta) {
  return drawPolarLine(p0.getX(), p0.getY(), r, theta);
}

GPoint GDrawingSurface::drawPolarLine(double x0, double y0, double r,
                                      double theta) {
...
void GDrawingSurface::drawRect(double x, double y, double width,
                               double height) {
...
void GDrawingSurface::fillArc(double x, double y, double width, double height,
                              double start, double sweep) {
  GArc arc(x, y, width, height, start, sweep);
...
void GDrawingSurface::fillOval(double x, double y, double width,
                               double height) {
  GOval oval(x, y, width, height);
...
void GDrawingSurface::fillRect(double x, double y, double width,
                               double height) {
  GRect rect(x, y, width, height);
...
std::string GDrawingSurface::getForeground() const { return getColor(); }

int GDrawingSurface::getForegroundInt() const { return getColorInt(); }
...
int GDrawingSurface::getRGB(double x, double y) const { return getPixel(x, y); }
...
bool GDrawingSurface::isRepaintImmediately() const { return isAutoRepaint(); }
...
void GDrawingSurface::setForeground(int color) { setColor(color); }
...
void GDrawingSurface::setPixelARGB(double x, double y, int a, int r, int g,
                                   int b) {
  setPixelARGB(x, y, GColor::convertARGBToARGB(a, r, g, b));
}
...
void GForwardDrawingSurface::repaintRegion(int x, int y, int width,
                                           int height) {
  if (_forwardTarget) {
...
void GForwardDrawingSurface::setPixelARGB(double x, double y, int a, int r,
                                          int g, int b) {
  ensureForwardTarget();
 ...
 * @version 2018/10/01
 * - bug fix where output wasn't showing up on the console if main ended too
 * soon
 * @version 2018/09/23
...
QFunctionThread::QFunctionThread(GThunk func)
    : _func(func), _hasReturn(false), _returnValue(0) {
  // empty
}

QFunctionThread::QFunctionThread(GThunkInt func)
    : _funcInt(func), _hasReturn(true), _returnValue(0) {
  // empty
}

int QFunctionThread::returnValue() const { return _returnValue; }
...
/*static*/ void
GThread::ensureThatThisIsTheQtGuiThread(const std::string &message) {
  if (!iAmRunningOnTheQtGuiThread()) {
    error((message.empty() ? "" : (message + ": ")) +
          "Qt GUI system must be initialized from the application's main "
          "thread.");
  }
}
...
/*static*/ GThread *GThread::getQtMainThread() { return _qtMainThread; }

/*static*/ GThread *GThread::getStudentThread() { return _studentThread; }
...
/*static*/ GThread *
GThread::runInNewThreadAsync(GThunk func, const std::string &threadName) {
  GThreadQt *thread = new GThreadQt(func, threadName);
  thread->start();
  return thread;
}
...
/*static*/ void GThread::runOnQtGuiThread(GThunk func) {
  if (iAmRunningOnTheQtGuiThread()) {
    ...
    error("GThread::runOnQtGuiThread: Qt GUI thread has not been initialized "
          "properly. \n"
          "Make sure that the file containing your main() function #includes "
          "at least \n"
...
/*static*/ void GThread::runOnQtGuiThreadAsync(GThunk func) {
  if (iAmRunningOnTheQtGuiThread()) {
    ...
    error("GThread::runOnQtGuiThreadAsync: Qt GUI thread has not been "
...
void GThread::yield() { QThread::yieldCurrentThread(); }
...
GThreadQt::GThreadQt(QThread *qthread) : _qThread(qthread) {
  _hasReturn = false;
  _returnValue = 0;
  _allGThreadsQt[_qThread] = this;
}
...
int GThreadQt::getResult() const { return _returnValue; }

bool GThreadQt::isRunning() const { return _qThread->isRunning(); }
...
// while the desired behavior is to have the calling thread wait for this
// thread.
...
void GThreadQt::start() { _qThread->start(); }
...
GThreadStd::GThreadStd(std::thread *stdThread) : _stdThread(stdThread) {
  _hasReturn = false;
  _returnValue = 0;
  _allGThreadsStd[_stdThread] = this;
}
...
int GThreadStd::getResult() const { return _returnValue; }

bool GThreadStd::isRunning() const { return _stdThread != nullptr && _running; }
...
std::string GThreadStd::name() const { return _name; }
...
void GThreadStd::setName(const std::string &name) { _name = name; }
...
void GThreadStd::yield() { std::this_thread::yield(); }
...
/*static*/ const std::string GDiffImage::HIGHLIGHT_COLOR_DEFAULT =
    "#e000e0"; // 224, 0, 224

/*static*/ void GDiffImage::showDialog(const std::string &name1,
                                       GCanvas *image1,
...
GDiffImage::GDiffImage(const std::string &name1, GCanvas *image1,
                       const std::string &name2, GCanvas *image2) {
  ...
  _colorButton->setActionListener([this]() { chooseHighlightColor(); });
  _colorButton->setKeyListener(windowCloseLambda);

  _diffPixelsLabel =
      new GLabel("(" + std::to_string(diffPixelsCount) + " pixels differ)");
  ...
  _window->getCanvas()->setMouseListener([this](GEvent event) {
    ...
    _southPixelLabel->setText("(x=" + std::to_string(x) +
                              ", y=" + std::to_string(y) +
...
void GDiffImage::drawImages() {
  ...
  if (_highlightDiffsBox->isChecked()) {
    GThread::runOnQtGuiThreadAsync([this]() {
      ...
      int highlightColor =
          GColor::convertColorToRGB(_highlightColor) | 0xff000000;
      QImage *img1 = _image1->getQImage();
      QImage *img2 = _image2->getQImage();
      for (int y = 0; y < hmax; y++) {
        for (int x = 0; x < wmax; x++) {
          int pixel1 =
              (x < w1 && y < h1) ? (img1->pixel(x, y) & 0xffffffff) : 0;
          int pixel2 =
              (x < w2 && y < h2) ? (img2->pixel(x, y) & 0xffffffff) : 0;
...
GRadioButton::GRadioButton(const std::string &text, const std::string &group,
                           bool checked, QWidget *parent) {
  GThread::runOnQtGuiThread([this, text, group, checked, parent]() {
    _iqradioButton =
        new _Internal_QRadioButton(this, checked, getInternalParent(parent));
...
std::string GRadioButton::getActionEventType() const { return "change"; }
...
std::string GRadioButton::getType() const { return "GRadioButton"; }

QWidget *GRadioButton::getWidget() const {
  return static_cast<QWidget *>(_iqradioButton);
}

bool GRadioButton::isChecked() const { return _iqradioButton->isChecked(); }

bool GRadioButton::isSelected() const { return _iqradioButton->isChecked(); }

void GRadioButton::setChecked(bool checked) {
  GThread::runOnQtGuiThread(
      [this, checked]() { _iqradioButton->setChecked(checked); });
}

void GRadioButton::setSelected(bool selected) { setChecked(selected); }
...
void GRadioButton::toggle() { setChecked(!isChecked()); }

/* static */ QButtonGroup *
GRadioButton::getButtonGroup(const std::string &group) {
  if (!_buttonGroups.containsKey(group)) {
    GThread::runOnQtGuiThread(
        [group]() { _buttonGroups.put(group, new QButtonGroup()); });
  }
  return _buttonGroups[group];
}

_Internal_QRadioButton::_Internal_QRadioButton(GRadioButton *gradioButton,
                                               bool checked, QWidget *parent)
    : QRadioButton(parent), _gradioButton(gradioButton) {
  require::nonNull(gradioButton, "_Internal_QRadioButton::constructor");
  setObjectName(QString::fromStdString("_Internal_QRadioButton_" +
                                       std::to_string(gradioButton->getID())));
  setChecked(checked);
  // We handle the clicked signal rather than toggled because, in a radio button
  // group, the toggled signal will fire twice: once for the radio button
  // clicked, and once for the other button that was unchecked.
  connect(this, SIGNAL(clicked()), this, SLOT(handleClick()));
}

void _Internal_QRadioButton::detach() { _gradioButton = nullptr; }
...
GCheckBox::GCheckBox(const std::string &text, bool checked, QWidget *parent) {
  GThread::runOnQtGuiThread([this, checked, parent]() {
    _iqcheckBox =
        new _Internal_QCheckBox(this, checked, getInternalParent(parent));
  });
  setText(text);
  setVisible(false); // all widgets are not shown until added to a window
}
...
std::string GCheckBox::getActionEventType() const { return "change"; }

_Internal_QWidget *GCheckBox::getInternalWidget() const { return _iqcheckBox; }

std::string GCheckBox::getText() const {
  return _iqcheckBox->text().toStdString();
}

std::string GCheckBox::getType() const { return "GCheckBox"; }

QWidget *GCheckBox::getWidget() const {
  return static_cast<QWidget *>(_iqcheckBox);
}

bool GCheckBox::isChecked() const { return _iqcheckBox->isChecked(); }

bool GCheckBox::isSelected() const { return _iqcheckBox->isChecked(); }

void GCheckBox::setChecked(bool checked) {
  GThread::runOnQtGuiThread(
      [this, checked]() { _iqcheckBox->setChecked(checked); });
}

void GCheckBox::setSelected(bool selected) { setChecked(selected); }

void GCheckBox::setText(const std::string &text) {
  GThread::runOnQtGuiThread(
      [this, text]() { _iqcheckBox->setText(QString::fromStdString(text)); });
}

void GCheckBox::toggle() { setChecked(!isChecked()); }

_Internal_QCheckBox::_Internal_QCheckBox(GCheckBox *gcheckBox, bool checked,
                                         QWidget *parent)
    : QCheckBox(parent), _gcheckBox(gcheckBox) {
  require::nonNull(gcheckBox, "_Internal_QCheckBox::constructor");
  setObjectName(QString::fromStdString("_Internal_QCheckBox_" +
                                       std::to_string(gcheckBox->getID())));
  setChecked(checked);
  connect(this, SIGNAL(stateChanged(int)), this, SLOT(handleStateChange(int)));
}

void _Internal_QCheckBox::detach() { _gcheckBox = nullptr; }
...
GTable::GTable(int rows, int columns, double width, double height,
               QWidget *parent)
    : _iqtableview(nullptr), _columnHeaderStyle(GTable::COLUMN_HEADER_NONE) {
  GThread::runOnQtGuiThread([this, rows, columns, parent]() {
    _iqtableview = new _Internal_QTableWidget(this, rows, columns,
                                              getInternalParent(parent));
    _iqtableview->setSelectionMode(QAbstractItemView::SingleSelection);
    _globalCellStyle = TableStyle::unset();
  });
  require::nonNegative2D(rows, columns, "GTable::constructor", "rows",
                         "columns");
  require::nonNegative2D(width, height, "GTable::constructor", "width",
                         "height");
  setVisible(false); // all widgets are not shown until added to a window
}
...
void GTable::autofitColumnWidths() {
  GThread::runOnQtGuiThread(
      [this]() { _iqtableview->resizeColumnsToContents(); });
}
...
void GTable::checkIndex(const std::string &member, int row, int column) const {
  require::inRange2D(row, column, 0, 0, numRows(), numCols(),
                     "GTable::" + member, "row", "column");
}
...
void GTable::clear() {
  GThread::runOnQtGuiThread([this]() {
    _iqtableview->clear();
    // for some reason, clearing a table also wipes the Excel-style column
    // headers
    updateColumnHeaders();
  });
}
...
void GTable::clearSelection() {
  GThread::runOnQtGuiThread([this]() { _iqtableview->clearSelection(); });
}
...
void GTable::ensureDefaultFormatting() const {
  ...
  if (!_defaultCellStyle.isSet()) {
    ...
    _defaultCellStyle.font =
        GFont::toFontString(thisHack->_iqtableview->font());
    _defaultCellStyle.alignment =
        ALIGN_LEFT; // TODO: ask cell for its alignment
  }
}
...
std::string GTable::get(int row, int column) const {
  ...
  return _iqtableview->model()
      ->data(_iqtableview->model()->index(row, column))
      .toString()
      .toStdString();
}
...
_Internal_QWidget *GTable::getInternalWidget() const { return _iqtableview; }
...
int GTable::getSelectedColumn() const { return getSelectedCell().col; }

int GTable::getSelectedRow() const { return getSelectedCell().row; }

std::string GTable::getType() const { return "GTable"; }
...
int GTable::height() const { return numRows(); }
...
bool GTable::isEditable() const {
  return (_iqtableview->editTriggers() & QAbstractItemView::NoEditTriggers) !=
         0;
}

int GTable::numCols() const { return _iqtableview->model()->columnCount(); }

int GTable::numRows() const { return _iqtableview->model()->rowCount(); }

void GTable::removeTableListener() {
  removeEventListeners({"table", "tableupdate", "tableselect", "tableeditbegin",
                        "tablereplacebegin", "tablecut", "tablecopy",
                        "tablepaste"});
}
...
void GTable::resize(int newNumRows, int newNumCols) {
  require::nonNegative2D(newNumRows, newNumCols, "GTable::resize", "rows",
                         "columns");
      ...
      for (int row = 0; row < newNumRows; row++) {
        for (int col = 0; col < newNumCols; col++) {
          if (row >= oldNumRows || col >= oldNumCols) {
            // figure out appropriate style (row, col, global, etc.) and apply
            // it
...
void GTable::select(int row, int column) {
  ...
  GThread::runOnQtGuiThread([this, row, column]() {
    ...
    _iqtableview->selectionModel()->select(index,
...
void GTable::setBackground(int rgb) {
  ...
  GThread::runOnQtGuiThread([this, rgb]() {
    // save this background color in the global cell style (for later cells on
    // resize() etc)
    ensureDefaultFormatting();
...
void GTable::setCellAlignment(int row, int column,
...
void GTable::setCellAlignmentInternal(int row, int column,
                                      HorizontalAlignment alignment) {
...
void GTable::setCellFontInternal(int row, int column, const std::string &font) {
  _iqtableview->item(row, column)
      ->setFont(GFont::toQFont(_iqtableview->font(), font));
}
...
void GTable::setColor(int rgb) { setForeground(rgb); }

void GTable::setColor(const std::string &color) { setForeground(color); }
...
void GTable::setColumnBackground(int column, int rgb) {
  ...
  GThread::runOnQtGuiThread([this, column, rgb]() {
    // save this background color in the column style (for later cells on
    // resize() etc)
    ensureColumnStyle(column);
...
void GTable::setColumnForeground(int column, int rgb) {
  ...
  GThread::runOnQtGuiThread([this, column, rgb]() {
    // save this foreground color in the column style (for later cells on
    // resize() etc)
    ensureColumnStyle(column);
...
void GTable::setFont(const QFont &font) { setFont(GFont::toFontString(font)); }
...
void GTable::setForeground(int rgb) {
  GThread::runOnQtGuiThread([this, rgb]() {
    ...
    // save this foreground color in the global cell style (for later cells on
    // resize() etc)
    ensureDefaultFormatting();
...
void GTable::setHorizontalAlignment(HorizontalAlignment alignment) {
  GThread::runOnQtGuiThread([this, alignment]() {
    // save this alignment in the global cell style (for later cells on resize()
    // etc)
    ensureDefaultFormatting();
...
void GTable::setRowBackground(int row, int rgb) {
  ...
  // save this background color in the row style (for later cells on resize()
  // etc)
...
void GTable::setRowForeground(int row, int rgb) {
  ...
  // save this foreground color in the row style (for later cells on resize()
  // etc)
  GThread::runOnQtGuiThread([this, row, rgb]() {
    ensureRowStyle(row);
...
void GTable::setRowHeight(int row, double height) {
  ...
  GThread::runOnQtGuiThread(
      [this, row, height]() { _iqtableview->setRowHeight(row, (int)height); });
}
...
void GTable::setTableListener(GEventListener func) {
  setEventListeners({"table", "tableupdate", "tableselect", "tableeditbegin",
                     "tablereplacebegin", "tablecut", "tablecopy",
                     "tablepaste"},
                    func);
}
...
int GTable::width() const { return numCols(); }

_Internal_QItemDelegate::_Internal_QItemDelegate(QObject *parent)
    : QStyledItemDelegate(parent), _editor(nullptr) {
  // empty
}

QWidget *
_Internal_QItemDelegate::createEditor(QWidget *parent,
...
void _Internal_QItemDelegate::destroyEditor(QWidget *editor,
                                            const QModelIndex &index) const {
  _Internal_QItemDelegate *hack = (_Internal_QItemDelegate *)this;
...
QWidget *_Internal_QItemDelegate::getEditor() const { return _editor; }

_Internal_QTableWidget::_Internal_QTableWidget(GTable *gtable, int rows,
                                               int columns, QWidget *parent)
    : QTableWidget(rows, columns, parent), _gtable(gtable), _delegate(nullptr) {
  require::nonNull(gtable, "_Internal_QTableWidget::constructor");
  setObjectName(QString::fromStdString("_Internal_QTableWidget_" +
                                       std::to_string(gtable->getID())));
  ...
  connect(this, SIGNAL(cellDoubleClicked(int, int)), this,
          SLOT(handleCellDoubleClick(int, int)));
  connect(
      this->selectionModel(),
      SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),
      this,
      SLOT(handleSelectionChange(const QItemSelection &,
                                 const QItemSelection &)));
}

void _Internal_QTableWidget::detach() { _gtable = nullptr; }

bool _Internal_QTableWidget::edit(const QModelIndex &index,
  ...
  if (result) {
    bool isEdit = _lastKeyPressed == 0 || _lastKeyPressed == Qt::Key_F2 ||
                  _lastKeyPressed == Qt::Key_Tab;
    _lastKeyPressed = 0;
    if (isEdit) {
      fireTableEvent(TABLE_EDIT_BEGIN, "tableeditbegin", index.row(),
                     index.column());
    } else {
      fireTableEvent(TABLE_REPLACE_BEGIN, "tablereplacebegin", index.row(),
                     index.column());
    }
  }
  return result;
...
void _Internal_QTableWidget::fireTableEvent(EventType eventType,
                                            const std::string &eventName,
                                            int row, int col) {
  if (!_gtable) {
    return;
  ...
  GEvent tableEvent(
      ...
      /* source */ _gtable);
  if (row < 0 && col < 0) {
    tableEvent.setRowAndColumn(_gtable->getSelectedRow(),
                               _gtable->getSelectedColumn());
  } else {
...
void _Internal_QTableWidget::closeEditor(
...
void _Internal_QTableWidget::handleCellDoubleClick(int /*row*/,
                                                   int /*column*/) {
  if (!_gtable) {
    return;
  }
...
void _Internal_QTableWidget::handleSelectionChange(
...
std::string GColorChooser::showDialog(const std::string &title,
                                      int initialColor) {
  return showDialog(/* parent */ (QWidget *)nullptr, title, initialColor);
}

std::string GColorChooser::showDialog(GWindow *parent, const std::string &title,
                                      int initialColor) {
  return showDialog(parent ? parent->getWidget() : nullptr, title,
                    GColor::convertRGBToColor(initialColor));
}

std::string GColorChooser::showDialog(QWidget *parent, const std::string &title,
                                      int initialColor) {
  return showDialog(parent, title, GColor::convertRGBToColor(initialColor));
}

std::string GColorChooser::showDialog(const std::string &title,
                                      const std::string &initialColor) {
  return showDialog(/* parent */ (QWidget *)nullptr, title, initialColor);
}
...
std::string GColorChooser::showDialog(QWidget *parent, const std::string &title,
  ...
  QColor initialQColor =
      initialColor.empty() ? Qt::white : GColor::toQColor(initialColor);
  std::string result = "";
  GThread::runOnQtGuiThread([parent, title, initialQColor, &result]() {
    QColor selectedColor = QColorDialog::getColor(
        initialQColor, parent, QString::fromStdString(title));
...
void GTextArea::appendFormattedText(const std::string &text,
...
void GTextArea::clearText() {
  GThread::runOnQtGuiThread([this]() { _iqtextedit->clear(); });
}
...
_Internal_QWidget *GTextArea::getInternalWidget() const { return _iqtextedit; }
...
GDimension GTextArea::getRowColumnSize() const {
  ...
  return GDimension(metrics.width(QString::fromStdString("mmmmmmmmmm")) / 10.0,
...
std::string GTextArea::getType() const { return "GTextArea"; }

QWidget *GTextArea::getWidget() const {
  return static_cast<QWidget *>(_iqtextedit);
}

bool GTextArea::isContextMenuEnabled() const { return _contextMenuEnabled; }

bool GTextArea::isEditable() const { return !_iqtextedit->isReadOnly(); }
...
void GTextArea::selectAll() {
  GThread::runOnQtGuiThread([this]() { _iqtextedit->selectAll(); });
}
...
void GTextArea::setEditable(bool value) {
  GThread::runOnQtGuiThread(
      [this, value]() { _iqtextedit->setReadOnly(!value); });
}

void GTextArea::setHtml(const std::string &html) {
  GThread::runOnQtGuiThread(
      [this, html]() { _iqtextedit->setHtml(QString::fromStdString(html)); });
}

void GTextArea::setLineWrap(bool wrap) {
  GThread::runOnQtGuiThread([this, wrap]() {
    _iqtextedit->setLineWrapMode(wrap ? QTextEdit::WidgetWidth
                                      : QTextEdit::NoWrap);
  });
}
...
void GTextArea::setRowsColumns(int rows, int columns) {
  require::nonNegative2D(rows, columns, "GTextArea::setRowsColumns", "rows",
                         "columns");
  double desiredWidth = getRowColumnSize().getWidth() * columns;
  double desiredHeight = getRowColumnSize().getHeight() * rows;
...
void GTextArea::setText(const std::string &text) {
  GThread::runOnQtGuiThread(
      [this, text]() { _iqtextedit->setText(QString::fromStdString(text)); });
}
...
_Internal_QTextEdit::_Internal_QTextEdit(GTextArea *gtextArea, QWidget *parent)
    : QTextEdit(parent), _gtextarea(gtextArea) {
  require::nonNull(gtextArea, "_Internal_QTextEdit::constructor");
  setObjectName(QString::fromStdString("_Internal_QTextEdit_" +
                                       std::to_string(gtextArea->getID())));
  ensureCursorVisible();
  setFocusPolicy(Qt::StrongFocus);
...
void _Internal_QTextEdit::detach() { _gtextarea = nullptr; }
...
GScrollPane::GScrollPane(GInteractor *interactor, QWidget *parent)
    : _iqscrollarea(nullptr), _interactor(interactor),
...
GInteractor *GScrollPane::getInteractor() const { return _interactor; }

_Internal_QWidget *GScrollPane::getInternalWidget() const {
  return _iqscrollarea;
}

std::string GScrollPane::getType() const { return "GScrollPane"; }
...
_Internal_QScrollArea::_Internal_QScrollArea(GScrollPane *gscrollpane,
                                             QWidget *parent)
    : QScrollArea(parent)
/*_gscrollpane(gscrollpane)*/ {
  require::nonNull(gscrollpane, "_Internal_QScrollArea::constructor");
  setObjectName(QString::fromStdString("_Internal_QScrollArea_" +
                                       std::to_string(gscrollpane->getID())));
}
...
GDownloader::GDownloader()
    : _manager(nullptr), _reply(nullptr), _httpStatusCode(0),
...
void GDownloader::downloadToFile(const std::string &url,
...
void GDownloader::downloadInternal() {
  GThread::runOnQtGuiThreadAsync([this]() {
    ...
    QNetworkRequest *request =
        new QNetworkRequest(QUrl(QString::fromStdString(_url)));
    ...
    for (std::string headerKey : _headers) {
      request->setRawHeader(QByteArray(headerKey.c_str()),
                            QByteArray(_headers[headerKey].c_str()));
    }
    ...
    // this seemingly won't be called ever (thanks, useless Qt networking
    // tutorials!)
    _reply->connect(_reply, SIGNAL(error(QNetworkReply::NetworkError)), this,
                    SLOT(fileDownloadError(QNetworkReply::NetworkError)));

#if QT_VERSION >= QT_VERSION_CHECK(5, 9, 0)
#if QT_CONFIG(ssl)
    _reply->connect(_reply, SIGNAL(sslErrors(QList<QSslError>)), this,
...
std::string GDownloader::getErrorMessage() const { return _lastErrorMessage; }
...
std::string
GDownloader::qtNetworkErrorToString(QNetworkReply::NetworkError nerror) {
  // http://doc.qt.io/qt-5/qnetworkreply.html#NetworkError-enum
  switch (nerror) {
  case QNetworkReply::ConnectionRefusedError:
    return "the remote server refused the connection (the server is not "
           "accepting requests)";
  case QNetworkReply::RemoteHostClosedError:
    return "the remote server closed the connection prematurely, before the "
  ...
  case QNetworkReply::OperationCanceledError:
    return "the operation was canceled via calls to abort() or close() before "
           "it was finished.";
  case QNetworkReply::SslHandshakeFailedError:
    return "the SSL/TLS handshake failed and the encrypted channel could not "
           "be established. The sslErrors() signal should have been emitted.";
  case QNetworkReply::TemporaryNetworkFailureError:
    return "the connection was broken due to disconnection from the network, "
           "however the system has initiated roaming to another access point. "
           "The request should be resubmitted and will be processed as soon as "
           "the connection is re-established.";
  case QNetworkReply::NetworkSessionFailedError:
    return "the connection was broken due to disconnection from the network or "
           "failure to start the network.";
  case QNetworkReply::BackgroundRequestNotAllowedError:
    return "the background request is not currently allowed due to platform "
           "policy.";
#if QT_VERSION >= QT_VERSION_CHECK(5, 9, 0)
  case QNetworkReply::TooManyRedirectsError:
    return "while following redirects, the maximum limit was reached. The "
           "limit is by default set to 50 or as set by "
           "QNetworkRequest::setMaxRedirectsAllowed(). (This value was "
           "introduced in 5.6.)";
  case QNetworkReply::InsecureRedirectError:
    return "while following redirects, the network access API detected a "
           "redirect from a encrypted protocol (https) to an unencrypted one "
           "(http). (This value was introduced in 5.6.)";
#endif // QT_VERSION
  case QNetworkReply::ProxyConnectionRefusedError:
    return "the connection to the proxy server was refused (the proxy server "
           "is not accepting requests)";
  case QNetworkReply::ProxyConnectionClosedError:
    return "the proxy server closed the connection prematurely, before the "
           "entire reply was received and processed";
  case QNetworkReply::ProxyNotFoundError:
    return "the proxy host name was not found (invalid proxy hostname)";
  case QNetworkReply::ProxyTimeoutError:
    return "the connection to the proxy timed out or the proxy did not reply "
           "in time to the request sent";
  case QNetworkReply::ProxyAuthenticationRequiredError:
    return "the proxy requires authentication in order to honour the request "
  ...
  case QNetworkReply::AuthenticationRequiredError:
    return "the remote server requires authentication to serve the content but "
           "the credentials provided were not accepted (if any)";
  case QNetworkReply::ContentReSendError:
    return "the request needed to be sent again, but this failed for example "
           "because the upload data could not be read a second time.";
  case QNetworkReply::ContentConflictError:
    return "the request could not be completed due to a conflict with the "
           "current state of the resource.";
  case QNetworkReply::ContentGoneError:
    return "the requested resource is no longer available at the server.";
  case QNetworkReply::InternalServerError:
    return "the server encountered an unexpected condition which prevented it "
           "from fulfilling the request.";
  case QNetworkReply::OperationNotImplementedError:
    return "the server does not support the functionality required to fulfill "
           "the request.";
  case QNetworkReply::ServiceUnavailableError:
    return "the server is unable to handle the request at this time.";
  case QNetworkReply::ProtocolUnknownError:
    return "the Network Access API cannot honor the request because the "
  ...
  case QNetworkReply::ProtocolFailure:
    return "a breakdown in protocol was detected (parsing error, invalid or "
...
void GDownloader::saveDownloadedData(const std::string &member,
  ...
  if (_reply) {
    ...
    if (nerror) {
      ...
      if (!outfile.open(QIODevice::WriteOnly)) {
        error("GDownloader::" + member + ": cannot open file " + filename +
              " for writing");
      }
...
void GDownloader::waitForDownload() {
  ...
  if (statusCode.isValid()) {
    ...
    _lastErrorMessage =
        _reply->attribute(QNetworkRequest::HttpReasonPhraseAttribute)
            .toString()
            .toStdString();
  } else {
    _httpStatusCode = -1;
...
GSlider::GSlider(int min, int max, int value, QWidget *parent) {
  ...
  GThread::runOnQtGuiThread([this, min, max, value, parent]() {
    _iqslider =
        new _Internal_QSlider(this, Qt::Horizontal, getInternalParent(parent));
...
GSlider::GSlider(Orientation orientation, int min, int max, int value,
                 QWidget *parent) {
  require::require(min <= max, "GSlider::constructor",
                   "min (" + std::to_string(min) +
                       ") cannot be greater than max (" + std::to_string(max) +
  ...
  GThread::runOnQtGuiThread([this, orientation, min, max, value, parent]() {
    _iqslider = new _Internal_QSlider(
        this, orientation == HORIZONTAL ? Qt::Horizontal : Qt::Vertical,
        getInternalParent(parent));
    _iqslider->setRange(min, max);
    _iqslider->setValue(value);
  });
...
std::string GSlider::getActionEventType() const { return "change"; }

_Internal_QWidget *GSlider::getInternalWidget() const { return _iqslider; }

int GSlider::getMajorTickSpacing() const { return _iqslider->tickInterval(); }

int GSlider::getMax() const { return _iqslider->maximum(); }

int GSlider::getMin() const { return _iqslider->minimum(); }

int GSlider::getMinorTickSpacing() const { return _iqslider->tickInterval(); }
...
std::string GSlider::getType() const { return "GSlider"; }

int GSlider::getValue() const { return _iqslider->value(); }
...
void GSlider::setMajorTickSpacing(int value) {
  GThread::runOnQtGuiThread(
      [this, value]() { _iqslider->setTickInterval(value); });
}
...
void GSlider::setMax(int max) {
  ...
  require::require(min <= max, "GSlider::setMax",
                   "max (" + std::to_string(max) +
                       ") cannot be less than min (" + std::to_string(min) +
                       ")");
  GThread::runOnQtGuiThread([this, max]() { _iqslider->setMaximum(max); });
}
...
void GSlider::setMin(int min) {
  ...
  require::require(min <= max, "GSlider::setMin",
                   "min (" + std::to_string(min) +
                       ") cannot be greater than max (" + std::to_string(max) +
                       ")");
  GThread::runOnQtGuiThread([this, min]() { _iqslider->setMinimum(min); });
}
...
void GSlider::setPaintTicks(bool value) {
  GThread::runOnQtGuiThread([this, value]() {
    _iqslider->setTickPosition(value ? QSlider::TicksBothSides
                                     : QSlider::NoTicks);
  });
}
...
void GSlider::setRange(int min, int max) {
  require::require(min <= max, "GSlider::setRange",
                   "min (" + std::to_string(min) +
                       ...
                       ")");
  GThread::runOnQtGuiThread(
      [this, min, max]() { _iqslider->setRange(min, max); });
}
...
void GSlider::setValue(int value) {
  ...
  GThread::runOnQtGuiThread([this, value]() { _iqslider->setValue(value); });
}

_Internal_QSlider::_Internal_QSlider(GSlider *gslider,
                                     Qt::Orientation orientation,
                                     QWidget *parent)
    : QSlider(orientation, parent), _gslider(gslider) {
  require::nonNull(gslider, "_Internal_QSlider::constructor");
  setObjectName(QString::fromStdString("_Internal_QSlider_" +
                                       std::to_string(gslider->getID())));
  connect(this, SIGNAL(valueChanged(int)), this, SLOT(handleChange(int)));
}

void _Internal_QSlider::detach() { _gslider = nullptr; }
...
/*static*/ bool GColor::hasAlpha(const std::string &color) {
  return static_cast<int>(color.length()) == 9 && color[0] == '#';
}
...
GSpacer::GSpacer(double width, double height, QWidget *parent)
    : _iqspacer(nullptr) {
  require::nonNegative2D(width, height, "GSpacer::constructor", "width",
                         "height");
  GThread::runOnQtGuiThread([this, width, height, parent]() {
    _iqspacer =
        new _Internal_QSpacer(this, width, height, getInternalParent(parent));
  });
  setVisible(false); // all widgets are not shown until added to a window
}
...
_Internal_QWidget *GSpacer::getInternalWidget() const { return _iqspacer; }

std::string GSpacer::getType() const { return "GSpacer"; }

QWidget *GSpacer::getWidget() const {
  return static_cast<QWidget *>(_iqspacer);
}

_Internal_QSpacer::_Internal_QSpacer(GSpacer *gspacer, double width,
                                     double height, QWidget *parent)
    : QWidget(parent) {
  require::nonNull(gspacer, "_Internal_QSpacer::constructor");
  setObjectName(QString::fromStdString("_Internal_QSpacer_" +
                                       std::to_string(gspacer->getID())));
...
GridLocationRange GridLocation::neighbors(int range, bool rowMajor) const {
  return GridLocationRange(row - range, col - range, row + range, col + range,
                           rowMajor);
}
...
int hashCode(const GridLocation &loc) { return hashCode(loc.row, loc.col); }
...
GridLocationRange::GridLocationRange(int startRow, int startCol, int endRow,
                                     int endCol, bool isRowMajor)
    : _start(startRow, startCol), _end(endRow, endCol),
      _isRowMajor(isRowMajor) {
  // empty
}

GridLocationRange::GridLocationRange(const GridLocation &startLoc,
                                     const GridLocation &endLoc,
                                     bool isRowMajor)
    : _start(startLoc), _end(endLoc), _isRowMajor(isRowMajor) {
  // empty
}
...
int GridLocationRange::endCol() const { return _end.col; }

const GridLocation &GridLocationRange::endLocation() const { return _end; }

int GridLocationRange::endRow() const { return _end.row; }

bool GridLocationRange::isEmpty() const { return _start > _end; }

bool GridLocationRange::isRowMajor() const { return _isRowMajor; }

int GridLocationRange::startCol() const { return _start.col; }

const GridLocation &GridLocationRange::startLocation() const { return _start; }

int GridLocationRange::startRow() const { return _start.row; }
...
bool stringNeedsQuoting(const std::string &str) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (isspace(ch))
      return false;
    if (STATIC_VARIABLE(STRING_DELIMITERS).find(ch) != std::string::npos)
      return true;
  }
  return false;
}
...
bool readQuotedString(std::istream &is, std::string &str, bool throwOnError) {
  ...
  if (ch == '\'' || ch == '"') {
    ...
    int endTrim = 0;
    while (is.get(ch) &&
           STATIC_VARIABLE(STRING_DELIMITERS).find(ch) == std::string::npos) {
      str += ch;
      if (!isspace(ch)) {
        endTrim = str.length();
      }
 ...
 * @version 2016/08/10
 * - added constructor support for std initializer_list usage, such as {"a",
 * "b", "c"}
 * @version 2016/08/04
 * - added operator >>
 * @version 2015/07/05
...
DawgLexicon::DawgLexicon()
    : _edges(nullptr), _start(nullptr), _edgeCount(0), _dawgWordsCount(0) {
  // empty
}
...
void DawgLexicon::addWordsFromFile(std::istream &input) {
  ...
  if (strncmp(firstFour, expected, 4) == 0) {
    if (_otherWords.size() != 0) {
      error("DawgLexicon::addWordsFromFile: Binary files require an empty "
            "lexicon");
    }
...
void DawgLexicon::addWordsFromFile(const std::string &filename) {
  ...
  if (input.fail()) {
    error("DawgLexicon::addWordsFromFile: Couldn't open lexicon file " +
          filename);
  }
  addWordsFromFile(input);
  input.close();
...
void DawgLexicon::insert(const std::string &word) { add(word); }

bool DawgLexicon::isEmpty() const { return size() == 0; }
...
int DawgLexicon::size() const { return _dawgWordsCount + _otherWords.size(); }
...
void DawgLexicon::readBinaryFile(std::istream &input) {
  ...
  input >> numBytes;
  input.get();
  if (input.fail() || strncmp(firstFour, expected, 4) != 0 || startIndex < 0 ||
      numBytes < 0) {
...
std::istream &operator>>(std::istream &is, DawgLexicon &lex) {
  std::string element;
  return stanfordcpplib::collections::readCollection(
      is, lex, element, /* descriptor */ "DawgLexicon::operator >>");
}
 ...
 */
static uint32_t my_ntohl(uint32_t arg) {
  uint32_t result = ((arg & 0xff000000) >> 24) | ((arg & 0x00ff0000) >> 8) |
...
int hashSeed() { return HASH_SEED; }

int hashMultiplier() { return HASH_MULTIPLIER; }

int hashMask() { return HASH_MASK; }

/*
 * Implementation notes: hashCode(int)
 * -----------------------------------
 * Hash code for integers masks off the sign bit, guaranteeing a nonnegative
 * value.
 */
int hashCode(int key) { return key & HASH_MASK; }

/*
 * Implementation notes: hashCode(other primitive types)
 * -----------------------------------------------------
 * Hash codes for all other primitive types forward to the hash code for
 * integers. This ensures that all hash codes get the proper masking treatment.
 *
 * Thanks to Jeremy Barenholtz for identifying that the original versions of
 * these functions, which just cast their arguments to integers, could lead to
 * negative results.
 */
int hashCode(bool key) { return hashCode(static_cast<int>(key)); }

int hashCode(char key) { return hashCode(static_cast<int>(key)); }

int hashCode(unsigned int key) { return hashCode(static_cast<int>(key)); }

int hashCode(long key) { return hashCode(static_cast<int>(key)); }

int hashCode(unsigned long key) { return hashCode(static_cast<int>(key)); }

int hashCode(short key) { return hashCode(static_cast<int>(key)); }

int hashCode(unsigned short key) { return hashCode(static_cast<int>(key)); }
 ...
 */
int hashCode(void *key) { return hashCode(reinterpret_cast<uintptr_t>(key)); }
...
int hashCode(const char *str) { return hashCode(str, strlen(str)); }
...
Lexicon::Lexicon() : _root(nullptr), _size(0), _removeFlag(false) {
  // empty
}

Lexicon::Lexicon(std::istream &input)
    : _root(nullptr), _size(0), _removeFlag(false) {
  addWordsFromFile(input);
}
...
void Lexicon::addWordsFromFile(const std::string &filename) {
  std::ifstream input(filename.c_str());
  if (input.fail()) {
    error("Lexicon::addWordsFromFile: Couldn't read from input file " +
          filename);
  }
  addWordsFromFile(input);
  input.close();
...
void Lexicon::insert(const std::string &word) { add(word); }
...
bool Lexicon::remove(const std::string &word) {
  ...
  return removeHelper(_root, scrubbed, /* originalWord */ scrubbed,
                      /* isPrefix */ false);
}
...
int Lexicon::size() const { return _size; }
 ...
 * Operators
 */
bool Lexicon::operator==(const Lexicon &lex2) const { return equals(lex2); }
...
Lexicon &Lexicon::operator+=(const Lexicon &lex2) { return addAll(lex2); }
...
Lexicon &Lexicon::operator*=(const Lexicon &lex2) { return retainAll(lex2); }

Lexicon &Lexicon::operator*=(std::initializer_list<std::string> list) {
  return retainAll(list);
}

Lexicon &Lexicon::operator-=(const Lexicon &lex2) { return removeAll(lex2); }
...
bool Lexicon::addHelper(TrieNode *&node, const std::string &word,
...
// pre: word is scrubbed to contain only lowercase a-z letters
bool Lexicon::containsHelper(TrieNode *node, const std::string &word,
                             bool isPrefix) const {
  if (!node) {
...
bool Lexicon::removeHelper(TrieNode *&node, const std::string &word,
  ...
  if (!node) {
    ...
    if (isPrefix) {
      ...
      removeSubtreeHelper(node,
                          originalWord); // removes from m_allWords, sets m_size
      node = nullptr;
    } else {
    ...
    bool result = removeHelper(node->child(word[0]), word.substr(1),
                               originalWord, isPrefix);

    // memory cleanup: if I wasn't a leaf but now am, and am not a word,
    // then I am now unneeded, so remove me too
    if (result && !wasLeaf && node && node->isLeaf() && !node->isWord()) {
      delete node;
      node = nullptr;
    }
    return result;
...
void Lexicon::removeSubtreeHelper(TrieNode *&node,
                                  const std::string &originalWord) {
  if (node) {
...
bool Lexicon::isDAWGFile(const std::string &filename) const {
  ...
  if (input.fail()) {
    error(
        std::string("Lexicon::addWordsFromFile: Couldn't open lexicon file ") +
        filename);
  }
...
std::istream &operator>>(std::istream &is, Lexicon &lex) {
  std::string element;
  return stanfordcpplib::collections::readCollection(
      is, lex, element, /* descriptor */ "Lexicon::operator >>");
}
...
bool isRunning() { return STATIC_VARIABLE(isRunning); }

std::string getContentType(const std::string &extension) {
  static Map<std::string, std::string>
      CONTENT_TYPE_MAP; // extension => MIME type
...
std::string getErrorMessage(int httpErrorCode) {
  ...
  if (ERROR_MESSAGE_MAP.isEmpty()) {
    ...
    ERROR_MESSAGE_MAP[420] =
        "HTTP ERROR 420: Enhance your calm; hey whatever man";
...
void sendResponse(const GEvent & /*event*/,
  ...
  if (contentTypeActual.empty()) {
    // TODO
    // contentTypeActual =
    // getContentType(getUrlExtension(event.getRequestURL()));
  }
  // TODO
  // stanfordcpplib::getPlatform()->httpserver_sendResponse(event.getRequestID(),
  // HTTP_ERROR_OK, contentTypeActual, responseText);
}
...
void sendResponseError(const GEvent & /*event*/, int httpErrorCode,
  ...
  // TODO
  // stanfordcpplib::getPlatform()->httpserver_sendResponse(event.getRequestID(),
  // httpErrorCode, STATIC_VARIABLE(CONTENT_TYPE_ERROR), errorMessageActual);
}

void sendResponseFile(const GEvent & /*event*/,
  ...
  // TODO
  // stanfordcpplib::getPlatform()->httpserver_sendResponseFile(event.getRequestID(),
  // contentType, responseFilePath);
}
 ...
 * @author Marty Stepp
 * @version 2017/11/12
 * - changed limited stream to throw error rather than raise SIGABRT for better
 * displaying
 * @version 2017/10/20
...
public:
  EchoingStreambuf(std::streambuf &buf, std::ostream &out)
      : instream(&buf), outstream(out), outputLimit(0), outputPrinted(0) {
    // outstream.rdbuf(&buf);
    setg(inBuffer, inBuffer, inBuffer);
    setp(outBuffer, outBuffer + BUFFER_SIZE);
  }

  ~EchoingStreambuf() { /* Empty */
  }

  virtual void setOutputLimit(int limit) { outputLimit = limit; }
...
public:
  LimitedStreambuf(std::streambuf &buf, int limit)
      : outstream(&buf), outputLimit(limit), outputPrinted(0) {
  ...
  virtual int overflow(int ch = EOF) {
    ...
    if (outputLimit > 0 && outputPrinted > outputLimit) {
      // error("excessive output printed");
      // outstream.setstate(std::ios::failbit | std::ios::badbit |
      // std::ios::eofbit); kill the program (use a signal rather than
      // error/exception so student won't try to catch it) error("Excessive
      // output printed; you may have an infinite loop in your code.");
      raise(SIGUSR1);
    } else {
      outstream.put(ch);
    }
...
void setOutputLimit(int limit) {
  ...
  LimitedStreambuf *limitedbufOut =
      new LimitedStreambuf(*std::cout.rdbuf(), limit);
  LimitedStreambuf *limitedbufErr =
      new LimitedStreambuf(*std::cerr.rdbuf(), limit);
 ...
 */
bool isHttpSuccess(int code) { return code >= 200 && code <= 299; }
} // namespace

iurlstream::iurlstream() : _url(""), _httpStatusCode(0) {
  // empty
}

iurlstream::iurlstream(const std::string &url) : _url(url), _httpStatusCode(0) {
  open(url);
}
...
std::string iurlstream::getErrorMessage() const { return _errorMessage; }
...
std::string iurlstream::getUrl() const { return _url; }
...
void iurlstream::open(const std::string &url) {
  ...
  if (downloader.hasError()) {
    ...
    clear();
    this->write(urlData.c_str(),
                static_cast<std::streamsize>(urlData.length()));
    this->seekg(0);
  }
}
 ...
 * - converted many funcs to take const string& rather than string for
 * efficiency
...
static bool recursiveMatch(const std::string &str, int sx,
                           const std::string &pattern, int px);
...
std::string defaultExtension(const std::string &filename,
                             const std::string &ext) {
  std::string extCopy = ext;
  bool force = (extCopy[0] == '*');
  if (force)
    extCopy = extCopy.substr(1);
  int dot = -1;
...
std::string getTempDirectory() { return platform::filelib_getTempDirectory(); }
...
bool matchFilenamePattern(const std::string &filename,
...
std::string openFileDialog(std::ifstream &stream, const std::string &title) {
...
std::string openFileDialog(const std::string &title, const std::string &path) {
  std::string filename = platform::file_openFileDialog(title, "load", path);
  if (filename == "")
    return "";
...
std::string openFileDialog(std::ofstream &stream, const std::string &title) {
  return openFileDialog(stream, title, "");
}
...
std::string openOnPath(std::ifstream &stream, const std::string &path,
                       const std::string &filename) {
...
std::string openOnPath(std::ofstream &stream, const std::string &path,
                       const std::string &filename) {
  Vector<std::string> paths;
  splitPath(path, paths);
  for (std::string dir : paths) {
...
std::string promptUserForFile(std::ifstream &stream, const std::string &prompt,
...
void rewindStream(std::istream &input) {
  ...
  input.seekg(0,
              std::ios::beg); // tells the stream to seek back to the beginning
}
...
bool writeEntireFile(const std::string &filename, const std::string &text,
                     bool append) {
  std::ofstream output;
  if (append) {
 ...
 * "lastTell" is std::streampos of the last byte that was read (this is used
...
ibitstream::ibitstream()
    : std::istream(nullptr), lastTell(0), curByte(0), pos(NUM_BITS_IN_BYTE) {
  this->fake = false;
}
...
int ibitstream::readBit() {
  if (!is_open()) {
    error("ibitstream::readBit: Cannot read a bit from a stream that is not "
          "open.");
  }
  ...
  if (this->fake) {
    ...
    // if just finished bits from curByte or if data read from stream after last
    // readBit()
...
void ibitstream::setFake(bool fake) { this->fake = fake; }

/* Member function ibitstream::size
 * --------------------------------
 * Seek to file end and use tell to retrieve position.
 * In order to not disrupt reading, we also record cur std::streampos and
...
long ibitstream::size() {
  ...
  std::streampos cur = tellg(); // save current std::streampos
 ...
 */
bool ibitstream::is_open() { return true; }

/* Constructor obitstream::obitstream
 * ----------------------------------
 * Each obitstream tracks 3 integers as private data.
 * "lastTell" is std::streampos of the last byte that was written (this is used
...
obitstream::obitstream()
    : std::ostream(nullptr), lastTell(0), curByte(0), pos(NUM_BITS_IN_BYTE) {
  this->fake = false;
}
 ...
 * rather than waiting for 8 bits.  This is because the client might make 3
 * writeBit calls and then start using << so we can't wait til full-byte
 * boundary to flush any partial-byte bits.
 */
void obitstream::writeBit(int bit) {
  if (bit != 0 && bit != 1) {
    error(std::string("obitstream::writeBit: must pass an integer argument of "
                      "0 or 1. You passed the integer ") +
...
void obitstream::setFake(bool fake) { this->fake = fake; }

/* Member function obitstream::size
 * --------------------------------
 * Seek to file end and use tell to retrieve position.
 * In order to not disrupt writing, we also record cur std::streampos and
 * re-seek to there before returning.
 */
long obitstream::size() {
  ...
  clear();                      // clear any error state
  std::streampos cur = tellp(); // save current std::streampos
 ...
 * open.  Subclasses can customize this if they'd like.
 */
bool obitstream::is_open() { return true; }
 ...
 * from disk.
 */
ifbitstream::ifbitstream() { init(&fb); }
...
void ifbitstream::open(const std::string &filename) { open(filename.c_str()); }

/* Member function ifbitstream::is_open
 * ------------------------------------
 * Determines whether the file stream is open.
 */
bool ifbitstream::is_open() { return fb.is_open(); }
 ...
 * to disk.
 */
ofbitstream::ofbitstream() { init(&fb); }
...
void ofbitstream::open(const char *filename) {
  ...
  if (endsWith(filename, ".cpp") || endsWith(filename, ".h") ||
    ...
    setstate(std::ios::failbit);
    error(std::string("ofbitstream::open: It is potentially dangerous to write "
                      "to file ") +
...
void ofbitstream::open(const std::string &filename) { open(filename.c_str()); }

/* Member function ofbitstream::is_open
 * ------------------------------------
 * Determines whether the file stream is open.
 */
bool ofbitstream::is_open() { return fb.is_open(); }
...
int TokenScanner::getChar() { return isp->get(); }

std::string TokenScanner::getInput() const { return buffer; }
...
TokenScanner::TokenType
TokenScanner::getTokenType(const std::string &token) const {
  if (token.empty()) {
...
void TokenScanner::ignoreComments() { ignoreCommentsFlag = true; }
...
void TokenScanner::ungetChar(int) { isp->unget(); }
...
int Base64encode(char *encoded, const char *string, int len) {
  ...
  for (i = 0; i < len - 2; i += 3) {
    ...
    *p++ =
        basis_64[((string[i + 1] & 0xF) << 2) | ((string[i + 2] & 0xC0) >> 6)];
    *p++ = basis_64[string[i + 2] & 0x3F];
  }
  if (i < len) {
    *p++ = basis_64[(string[i] >> 2) & 0x3F];
...
STATIC_CONST_VARIABLE_DECLARE(std::string, GETCHAR_DEFAULT_PROMPT,
                              "Enter a character: ")
STATIC_CONST_VARIABLE_DECLARE(std::string, GETCHAR_DEFAULT_REPROMPT,
                              "You must type a single character. Try again.")
STATIC_CONST_VARIABLE_DECLARE(std::string, GETINTEGER_DEFAULT_PROMPT,
                              "Enter an integer: ")
STATIC_CONST_VARIABLE_DECLARE(std::string, GETINTEGER_DEFAULT_REPROMPT,
                              "Illegal integer format. Try again.")
STATIC_CONST_VARIABLE_DECLARE(std::string, GETREAL_DEFAULT_PROMPT,
                              "Enter a number: ")
STATIC_CONST_VARIABLE_DECLARE(std::string, GETREAL_DEFAULT_REPROMPT,
                              "Illegal numeric format. Try again.")
STATIC_CONST_VARIABLE_DECLARE(std::string, GETYESORNO_DEFAULT_PROMPT,
                              "Try again: ")
...
char getChar(const std::string &prompt, const std::string &reprompt) {
  ...
  while (true) {
    ...
    std::cout.flush();
    std::cerr << (reprompt.empty() ? STATIC_VARIABLE(GETCHAR_DEFAULT_REPROMPT)
                                   : reprompt)
              << std::endl;
    std::cerr.flush();
    if (promptCopy.empty()) {
...
int getInteger(const std::string &prompt, const std::string &reprompt) {
  ...
  while (true) {
    ...
    if (!getline(std::cin, line)) {
      error(
          "getInteger: End of input reached while waiting for integer value.");
    }
    trimInPlace(line);
    ...
    std::cout.flush();
    std::cerr << (reprompt.empty()
                      ? STATIC_VARIABLE(GETINTEGER_DEFAULT_REPROMPT)
                      : reprompt)
              << std::endl;
    std::cerr.flush();
    if (promptCopy.empty()) {
...
void getLine(const std::string &prompt, std::string &out) {
  std::string promptCopy = prompt;
  appendSpace(promptCopy);
  std::cout << promptCopy;
...
double getReal(const std::string &prompt, const std::string &reprompt) {
  std::string promptCopy = prompt;
  appendSpace(promptCopy);
  double value = 0.0;
  while (true) {
    ...
    std::cout.flush();
    std::cerr << (reprompt.empty() ? STATIC_VARIABLE(GETREAL_DEFAULT_REPROMPT)
                                   : reprompt)
              << std::endl;
    std::cerr.flush();
    if (promptCopy.empty()) {
...
bool getYesOrNo(const std::string &prompt, const std::string &reprompt,
  ...
  while (true) {
    ...
    std::cout.flush();
    std::cerr << (reprompt.empty()
                      ? STATIC_VARIABLE(GETYESORNO_DEFAULT_REPROMPT)
                      : reprompt)
              << std::endl;
    std::cerr.flush();
    if (promptCopy.empty()) {
 ...
 * @author Marty Stepp
 * @version 2016/10/22
 * - changed openXmlDocument to print error message rather than crash on
 * file-not-found
 * @version 2016/10/14
...
namespace xmlutils {
int getAttributeInt(rapidxml::xml_node<> *node, const std::string &attrName,
                    int defaultValue) {
  std::string value =
      getAttribute(node, attrName, std::to_string(defaultValue));
  return stringToInteger(value);
}

bool getAttributeBool(rapidxml::xml_node<> *node, const std::string &attrName,
                      bool defaultValue) {
  std::string value =
      toLowerCase(getAttribute(node, attrName, boolToString(defaultValue)));
...
Vector<rapidxml::xml_node<> *> getChildNodes(rapidxml::xml_node<> *node,
...
rapidxml::xml_node<> *openXmlDocument(const std::string &filename,
                                      const std::string &documentNode) {
  std::string xmlFileText = readEntireFile(filename);
  char *buf = new char[xmlFileText.length() +
                       1024](); // *** memory leak (but MUST be heap-allocated)
  memset(buf, 0, xmlFileText.length() + 1024);
  strcpy(buf, xmlFileText.c_str());
  rapidxml::xml_document<char> *xmlDoc =
      new rapidxml::xml_document<char>; // *** memory leak (but MUST be
                                        // heap-allocated)
 ...
 * - removed static global Platform variable, replaced by getPlatform as needed
 * @version 2014/11/13
 * - ifdef'd out macroIntegerToDate function to remove unused function warning
 * on some platforms
 * @since 2014/10/31
 */
...
std::string getCppLibraryDocsUrl() { return STANFORD_CPP_LIB_DOCS_URL; }

std::string getCppLibraryVersion() { return STANFORD_CPP_LIB_VERSION; }

std::string getJavaBackEndVersion() { return "?"; }
...
std::string filelib_getDirectoryPathSeparator() { return "\\"; }

std::string filelib_getSearchPathSeparator() { return ";"; }
...
bool exitEnabled() { return STATIC_VARIABLE(isExitEnabled); }
...
#ifndef _WIN32
  // on Mac only, may need to change folder because of app's nested dir
  // structure
...
#ifdef SPL_HEADLESS_MODE
void runMainInThread(int (*mainFunc)(void)) { mainFunc(); }

void runMainInThread(std::function<int()> mainFunc) { mainFunc(); }

void runMainInThreadVoid(void (*mainFuncVoid)(void)) { mainFuncVoid(); }

void runMainInThreadVoid(std::function<void()> mainFuncVoid) { mainFuncVoid(); }
#else  // SPL_HEADLESS_MODE
...
#define exit __stanfordcpplib__exitLibrary
    ...
    std::ostringstream out;
    out << "Program tried to call exit(" << status << ") to quit. "
        << std::endl;
    out << "*** This function has been disabled; main should end through "
        << std::endl;
...
void filelib_deleteFile(const std::string &path) { remove(path.c_str()); }
...
#undef main
int main(int argc, char **argv) { return qMain(argc, argv); }
